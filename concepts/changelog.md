...
### <a name="data-policy-operations-api"></a>API de operações de diretiva de dados

| **Tipo de alteração** | **Versão** | **Descrição**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Adição        | beta        | Adicionada nova propriedade de **andamento** para [dataPolicyOperation](../api-reference/beta/resources/dataPolicyOperation). Especifica o progresso de uma operação.

### <a name="microsoft-teams-apis"></a>APIs do Microsoft Teams

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Adição |v1.0| Introduzido de tipo do novo recurso [equipe](/graph/api/resources/team?view=graph-rest-1.0).|
|Adição |v1.0| Introduzido de tipo de recurso novo [canal](/graph/api/resources/channel?view=graph-rest-1.0).|
|Adição |v1.0| Introduzido de tipo do novo recurso [teamsApp](/graph/api/resources/teamsapp?view=graph-rest-1.0).|
|Adição |v1.0| Introduzido de tipo do novo recurso [teamsAppDefinition](/graph/api/resources/teamsappdefinition?view=graph-rest-1.0).|
|Adição |v1.0| Introduzido de tipo do novo recurso [teamsAppInstallation](/graph/api/resources/teamsappinstallation?view=graph-rest-1.0).|
|Adição |v1.0| Introduzido de tipo do novo recurso [teamsAsyncOperation](/graph/api/resources/teamsasyncoperation?view=graph-rest-1.0). |
|Adição |v1.0| Introduzido [teamGuestSettings](/graph/api/resources/teamguestsettings?view=graph-rest-1.0)de novo tipo complexo. |
|Adição |v1.0| Introduzido [teamMemberSettings](/graph/api/resources/teammembersettings?view=graph-rest-1.0)de novo tipo complexo. |
|Adição |v1.0| Introduzido [teamMessagingSettings](/graph/api/resources/teammessagingsettings?view=graph-rest-1.0)de novo tipo complexo. |
|Adição |v1.0| Introduzido [teamFunSettings](/graph/api/resources/teamfunsettings?view=graph-rest-1.0)de novo tipo complexo. |
|Adição |v1.0| Introduzido nova ação de [Clone equipe](/graph/api/team-clone?view=graph-rest-1.0). |
|Adição |v1.0| Introduzido nova ação de [equipe de arquivamento](/graph/api/team-archive?view=graph-rest-1.0).|
|Adição |v1.0| Introduzido nova ação [Unarchive equipe](/graph/api/team-unarchive?view=graph-rest-1.0). |
|Adição         | Beta          | Adicionado o suportam de permissões de aplicativo à [equipe de clone](/graph/api/team-clone?view=graph-rest-beta). |
...
### <a name="riskyusers-apis"></a>APIs de RiskyUsers

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Adição |beta| Introduzido [API riskyUsers](/graph/api/resources/riskyuser?view=graph-rest-beta), que representa os usuários do Azure AD que estão em risco, conforme detectada pelo proteção de identidade do Windows Azure AD. |


### <a name="signin-apis"></a>APIs de entrar

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Alteração   |beta| Renomeado a propriedade `conditionalAccessPolicies` para `appliedConditionalAccessPolicy`.|
|Adição |beta| Introduzido propriedades de risco adicional na [API de entrar](/graph/api/resources/signin?view=graph-rest-beta), incluindo `riskDetail`, `riskLevelAggregated`, `riskLevelDuringSignIn`, `riskEventTypes`, e `riskState`.|
|Adição |beta| Introduzido propriedades adicionais entrar na [API de entrar](/graph/api/resources/signin?view=graph-rest-beta), incluindo `authenticationProcessingDetails`, `originalRequestID`, `isInteractive`, `tokenIssuerName`, `tokenIssuerType`, `correlationId`, e `processingTimeinMilliseconds`.|
|Remoção   |beta| Removida a propriedade `isRisky`.|
...
| **Tipo de alteração** | **Versão** | **Descrição**                  |
|:------------|:--------|:-----------------------------------------|
| Adição    | Beta   | Recurso de adicionado [consulta delta](delta-query-overview.md) para [directoryObject](/graph/api/directoryobject-delta?view=graph-rest-beta) |
| Alteração      | v1.0 e beta  | Comportamento alternativo para retornar propriedades alteradas somente em resposta JSON para [usuários](/graph/api/user-delta?view=graph-rest-1.0) e [grupos](/graph/api/group-delta?view=graph-rest-1.0). |
| Adição    | v1.0   | Adicionada a função [delta](/graph/api/directoryrole-delta?view=graph-rest-1.0) para [directoryRole](/graph/api/resources/directoryrole?view=graph-rest-1.0) dar suporte ao [alterar usando delta consulta de rastreamento](delta-query-overview.md). |

### <a name="directory-apis"></a>APIs de diretório

| **Tipo de alteração** | **Versão**   | **Descrição**                          | 
| :-------------- | :------------ | :--------------------------------------- | 
| Adição | Beta | Adicionada a propriedade **licenseAssignmentStates** à entidade de [usuário](/graph/api/resources/user?view=graph-rest-beta) para o [Grupo com base em licenciamento](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-licensing-whatis-azure-portal).|
| Adição | Beta | Adicionado o recurso **licenseAssignmentState** por [Grupo com base em licenciamento](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-licensing-whatis-azure-portal).| 
| Adição | Beta | Adicionadas as propriedades **assignedLicenses**, **licenseProcessingState**, **hasMembersWithLicenseErrors** e **membersWithLicenseErrors** à entidade de [grupo](/graph/api/resources/group?view=graph-rest-beta) por [Grupo com base em licenciamento](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-licensing-whatis-azure-portal).|

### <a name="microsoft-intune-apis"></a>APIs do Microsoft Intune

|Tipo de alteração|Versão|Descrição|
|:---|:---|:---|
|Adição|v1.0|Adicionada a propriedade **tenantLockdownRequireNetworkDuringOutOfBoxExperience** à entidade [windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-1.0)|
|Adição|v1.0|Adicionada a propriedade **v12_0** para o tipo de [iosMinimumOperatingSystem](/graph/api/resources/intune-apps-iosminimumoperatingsystem?view=graph-rest-1.0) complexo|
|Adição|beta|Adicionada a propriedade **lastReportAggregationDateTime** à entidade [deviceManagement](/graph/api/resources/intune-shared-devicemanagement?view=graph-rest-beta)|
...
|Adição|beta|Adicionada a função [managedDeviceEnrollmentAbandonmentDetails](/graph/api/intune-troubleshooting-reportroot-manageddeviceenrollmentabandonmentdetails?view=graph-rest-beta) em [reportRoot](/graph/api/resources/intune-deviceconfig-reportroot?view=graph-rest-beta) |
|Exclusão|beta|Removida a propriedade **subjectAlternativeNameType** da entidade [androidForWorkCertificateProfileBase](/graph/api/resources/intune-deviceconfig-androidforworkcertificateprofilebase?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **subjectAlternativeNameType** à entidade [androidForWorkPkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-androidforworkpkcscertificateprofile?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **certificateStore**, **customSubjectAlternativeNames** e **subjectAlternativeNameType** à entidade [androidForWorkScepCertificateProfile](/graph/api/resources/intune-deviceconfig-androidforworkscepcertificateprofile?view=graph-rest-beta)|
|Exclusão|beta|Removida a propriedade **subjectAlternativeNameType** da entidade [androidWorkProfileCertificateProfileBase](/graph/api/resources/intune-deviceconfig-androidworkprofilecertificateprofilebase?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **subjectAlternativeNameType** à entidade [androidWorkProfilePkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-androidworkprofilepkcscertificateprofile?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **certificateStore**, **customSubjectAlternativeNames** e **subjectAlternativeNameType** à entidade [androidWorkProfileScepCertificateProfile](/graph/api/resources/intune-deviceconfig-androidworkprofilescepcertificateprofile?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **legacyPcManangementEnabled** à entidade [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)|
|Exclusão|beta|Removida a propriedade **pinRequiredOnLaunchInsteadOfBiometric** da entidade [managedAppProtection](/graph/api/resources/intune-mam-managedappprotection?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **roleScopeTagIds** à entidade [managedDeviceMobileAppConfiguration](/graph/api/resources/intune-apps-manageddevicemobileappconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **applicationGuardEnabledOptions** à entidade [windows10EndpointProtectionConfiguration](/graph/api/resources/intune-deviceconfig-windows10endpointprotectionconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **selectedMobileAppIds** à entidade [windows10EnrollmentCompletionPageConfiguration](/graph/api/resources/intune-onboarding-windows10enrollmentcompletionpageconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionado o **engagedRestartDeadlineInDays**, **engagedRestartSnoozeScheduleInDays**, **engagedRestartTransitionScheduleInDays**, **autoRestartNotificationDismissal**, **scheduleRestartWarningInHours** e ** scheduleImminentRestartWarningInMinutes** propriedades à entidade [windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **preSharedKey** e **meteredConnectionLimit** à entidade [windowsWifiConfiguration](/graph/api/resources/intune-deviceconfig-windowswificonfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade de navegação **intuneBrandingProfiles** à entidade [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **v6_0**, **v7_0**, **v7_1**, **v8_0**, **v8_1** e **v9_0** ao tipo complexo [androidMinimumOperatingSystem](/graph/api/resources/intune-apps-androidminimumoperatingsystem?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **v12_0** para o tipo de [iosMinimumOperatingSystem](/graph/api/resources/intune-apps-iosminimumoperatingsystem?view=graph-rest-beta) complexo|
|Exclusão|beta|Removida a propriedade **runAsLoggedOnUser** do tipo [win32LobAppPowerShellScriptDetection](/graph/api/resources/intune-apps-win32lobapppowershellscriptdetection?view=graph-rest-beta) complexa|
|Adição|beta|Adicionada a propriedade **lastUpdateDateTime** para o tipo de [osVersionCount](/graph/api/resources/intune-devices-osversioncount?view=graph-rest-beta) complexo|
|Adição|beta|Adicionada a propriedade **lastUpdateDateTime** para o tipo de [windowsMalwareCategoryCount](/graph/api/resources/intune-devices-windowsmalwarecategorycount?view=graph-rest-beta) complexo|
|Adição|beta|Adicionada a propriedade **lastUpdateDateTime** para o tipo de [windowsMalwareExecutionStateCount](/graph/api/resources/intune-devices-windowsmalwareexecutionstatecount?view=graph-rest-beta) complexo|
|Adição|beta|Adicionada a propriedade **lastUpdateDateTime** para o tipo de [windowsMalwareNameCount](/graph/api/resources/intune-devices-windowsmalwarenamecount?view=graph-rest-beta) complexo|
|Adição|beta|Adicionada a propriedade **lastUpdateDateTime** para o tipo de [windowsMalwareStateCount](/graph/api/resources/intune-devices-windowsmalwarestatecount?view=graph-rest-beta) complexo|

### <a name="privileged-identity-management-apis"></a>APIs de Privileged Identity Management

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Alteração | beta | Altere a entidade [privilegedapproval](/graph/api/resources/privilegedapproval?view=graph-rest-beta) .|
| Adição | beta | Adicionada a entidade [privilegedroleassignmentrequest](/graph/api/resources/privilegedroleassignmentrequest?view=graph-rest-beta) e seguir métodos e ações:<br> [List](/graph/api/privilegedroleassignmentrequest-list?view=graph-rest-beta) <br> [Create](/graph/api/privilegedroleassignmentrequest-post?view=graph-rest-beta) <br> [Cancel](/graph/api/privilegedroleassignmentrequest-cancel?view=graph-rest-beta) <br> [Minha](/graph/api/privilegedroleassignmentrequest-my?view=graph-rest-beta) |
| Adição | beta | Adicionado a [atualização](/graph/api/privilegedrolesettings-update?view=graph-rest-beta) para [privilegedRoleSettings](/graph/api/resources/privilegedrolesettings?view=graph-rest-beta)|
| Remoção |beta| Preterido o [Self ativar a atribuição de função](/graph/api/privilegedrole_selfactivate?view=graph-rest-beta)|

### <a name="microsoft-teams-apis"></a>APIs do Microsoft Teams

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Adição|beta|Adicionado o suporte de permissões do aplicativo para a [equipe de arquivamento](/graph/api/team-archive?view=graph-rest-beta) e [unarchive equipe](/graph/api/team-unarchive?view=graph-rest-beta) APIs.|

### <a name="outlook-contacts"></a>Contatos do Outlook

| **Tipo de alteração** | **Versão** | **Descrição**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Remoção         | v1.0        | Isso é uma correção à documentação: removido a propriedade **sinalizador** do tópico de entidade [Contatos](/graph/api/resources/contact?view=graph-rest-1.0) . A propriedade nunca foi feita disponível na entidade **Contatos** .|

### <a name="reports-apis"></a>APIs de relatórios
| Tipo de alteração | Versão | Descrição                              |
|:------------|:--------|:-----------------------------------------|
| Adição    | v1.0    | Adicionada a propriedade **ID do Site** ao [getSharePointSiteUsageDetail](/graph/api/reportroot-getsharepointsiteusagedetail?view=graph-rest-1.0). |

## <a name="september-2018"></a>Setembro de 2018

### <a name="calls-and-online-meetings-api"></a>API de reuniões online e chamadas

| **Tipo de alteração** | **Versão** | **Descrição**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Alteração          | Beta        | O recurso do [aplicativo](/graph/api/resources/application?view=graph-rest-beta) foi atualizado para adicionar um conjunto de chamadas. |
| Alteração          | Beta        | O recurso de [operação](/graph/api/resources/operation?view=graph-rest-beta) foi atualizado para oferecer suporte a chamadas de longa execução e APIs de reuniões. |
| Adição        | Beta        | Adicionado o recurso [chamada](/graph/api/resources/call?view=graph-rest-beta) de gerenciamento de chamadas de áudio/vídeo (inicialmente, em Microsoft Teams), incluindo APIs para a [criação de chamadas](/graph/api/application-post-calls?view=graph-rest-beta), [Recuperando um uma chamada](/graph/api/call-get?view=graph-rest-beta), [excluindo (desligar) uma chamada](/graph/api/call-delete?view=graph-rest-beta), que [pode atender uma chamada](/graph/api/call-answer?view=graph-rest-beta), rejeitando [um chamar](/graph/api/call-reject?view=graph-rest-beta), [redirecionar uma chamada](/graph/api/call-redirect?view=graph-rest-beta)e [transferir uma chamada](/graph/api/call-transfer?view=graph-rest-beta). Também adicionamos APIs para oferecer suporte a [cenários IVR](/graph/api/resources/calls-api-ivr-overview?view=graph-rest-beta): [reproduzir um prompt](/graph/api/call-playprompt?view=graph-rest-beta), [uma chamada de gravação](/graph/api/call-record?view=graph-rest-beta), [Cancelar o processamento de mídia](/graph/api/call-cancelmediaprocessing?view=graph-rest-beta)e [Inscreva-se para tocar tom notificações](/graph/api/call-subscribetotone?view=graph-rest-beta). |
| Adição        | Beta        | Adicionadas as APIs e o recurso de [participante](/graph/api/resources/call?view=graph-rest-beta) para gerenciar os participantes em chamadas de áudio/vídeo e reuniões, incluindo [a recuperação de um objeto participante](/graph/api/participant-get?view=graph-rest-beta), [Configurando mixer de áudio para um participante](/graph/api/participant-configuremixer?view=graph-rest-beta), tirar o som [uma](/graph/api/participant-mute?view=graph-rest-beta) ou [todas as ](/graph/api/participant-muteall?view=graph-rest-beta)dos participantes, a [recuperação de uma lista dos participantes](/graph/api/call-list-participants?view=graph-rest-beta) em uma reunião de chamada/e [convidando participantes](/graph/api/participant-invite?view=graph-rest-beta) para uma chamada/reunião. |
| Adição        | Beta        | APIs adicionados para aplicativos para gerenciar e participar de chamadas e reuniões, incluindo a capacidade de [compartilhar conteúdo](/graph/api/call-changescreensharingrole?view=graph-rest-beta), [Ativar e desativar o mudo](/graph/api/call-unmute?view=graph-rest-beta)e [atualizar os metadados associados a uma chamada](/graph/api/call-updatemetadata?view=graph-rest-beta). |
| Adição        | Beta        | Adicionadas as APIs e o recurso de [grupo de roteamento de áudio](/graph/api/resources/audioroutinggroup?view=graph-rest-beta) para o gerenciamento de rotas de áudio privadas entre participantes em uma conversa com vários participantes, incluindo [criação de grupos de roteamento de áudio](/graph/api/call-post-audioroutinggroups?view=graph-rest-beta), [Recuperando uma lista deles](/graph/api/audioroutinggroup-get?view=graph-rest-beta)e [Atualizando](/graph/api/audioroutinggroup-update?view=graph-rest-beta) e [Excluindo](/graph/api/audioroutinggroup-delete?view=graph-rest-beta) -los. |
| Adição        | Beta        | Adicionadas as APIs e o recurso de [reunião online](/graph/api/resources/audioroutinggroup?view=graph-rest-beta) para gerenciar reuniões online do Microsoft Teams. Inicialmente, há apenas uma API para reuniões online, para [recuperar um objeto de reunião online](/graph/api/onlinemeeting-get?view=graph-rest-beta). Um recurso relacionado para as [informações de conferência de áudio](/graph/api/resources/audioconferencing?view=graph-rest-beta) associadas a uma reunião (por exemplo, discada URL, códigos de acesso e números de telefone) também foi adicionado. |
| Adição        | Beta        | Muitas das chamadas e reuniões APIs demoram para ser concluída, portanto recursos para essas operações de longa execução foram adicionados: [operações específicas de chamada](/graph/api/resources/commsoperation?view=graph-rest-beta), a [execução de prompts de áudio](/graph/api/resources/playpromptoperation?view=graph-rest-beta)e [gravação](/graph/api/resources/recordoperation?view=graph-rest-beta).  |

### <a name="dynamics-365-business-central-api"></a>API de Central de Business Dynamics 365

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição        | Beta          | Adicionado financials APIs para a Central de Business Dynamics 365. Para obter detalhes, consulte a [referência de API de finanças](/graph/api/resources/dynamics-graph-reference?view=graph-rest-beta)

### <a name="microsoft-graph-data-connect"></a>Conexão de dados do Microsoft Graph

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Adição         | Não aplicável| Introduziu a capacidade de acessar dados do Office 365 em massa. Para obter detalhes, consulte [dados do Microsoft Graph conectar (preview)](data-connect-overview.md).|
...
|Adição|beta|Adicionada a ação [atribuir](/graph/api/intune-cirrus-officeclientconfiguration-assign?view=graph-rest-beta) em [officeClientConfiguration](/graph/api/resources/intune-cirrus-officeclientconfiguration?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação **updatePrioritie** no conjunto de [officeClientConfiguration](/graph/api/resources/intune-cirrus-officeclientconfiguration?view=graph-rest-beta) |
...
|Adição|beta|Adicionada a ação **getTargetedUsersAndDevices** no conjunto de [deviceConfiguration](/graph/api/resources/intune-deviceconfig-deviceconfiguration?view=graph-rest-beta) |
...
|Exclusão|beta|Removida a propriedade **restrictedApps** da entidade [androidForWorkCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidforworkcompliancepolicy?view=graph-rest-beta)|
|Exclusão|beta|Removida a propriedade **restrictedApps** da entidade [androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-beta)|
|Alteração|beta|Alteradas as seguintes propriedades na entidade [appleVpnConfiguration](/graph/api/resources/intune-deviceconfig-applevpnconfiguration?view=graph-rest-beta) :<br/>**enablePerApp** de necessária para opcional<br/>|
|Adição|beta|Adicionadas as propriedades **disableProtectionOfManagedOutboundOpenInData** e **protectInboundDataFromUnknownSources** à entidade [defaultManagedAppProtection](/graph/api/resources/intune-mam-defaultmanagedappprotection?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **microsoftStoreForBusinessPortalSelection** à entidade [deviceAppManagement](/graph/api/resources/intune-shared-deviceappmanagement?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **passcodeMinutesOfInactivityBeforeScreenTimeout** à entidade [iosCompliancePolicy](/graph/api/resources/intune-deviceconfig-ioscompliancepolicy?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **useOAuth** à entidade [iosEasEmailProfileConfiguration](/graph/api/resources/intune-deviceconfig-ioseasemailprofileconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionado o **kioskModeBlockVolumeButtons**, **classroomForceRequestPermissionToLeaveClasses**, **keychainBlockCloudSync**, **pkiBlockOTAUpdates**, **privacyForceLimitAdTracking**, ** enterpriseBookBlockBackup**, **enterpriseBookBlockMetadataSync**, **airPrintBlocked**, **airPrintBlockCredentialsStorage**, **airPrintForceTrustedTLS**, **airPrintBlockiBeaconDiscovery**, ** blockSystemAppRemoval** e propriedades de **vpnBlockCreation** à entidade [iosGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-iosgeneraldeviceconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **disableProtectionOfManagedOutboundOpenInData** e **protectInboundDataFromUnknownSources** à entidade [iosManagedAppProtection](/graph/api/resources/intune-mam-iosmanagedappprotection?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **gatekeeperAllowedAppSource** à entidade [macOSCompliancePolicy](/graph/api/resources/intune-deviceconfig-macoscompliancepolicy?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **keychainBlockCloudSync**, **airPrintBlocked**, **airPrintForceTrustedTLS** e **airPrintBlockiBeaconDiscovery** à entidade [macOSGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-macosgeneraldeviceconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **deviceModel** e **deviceManufacturer** à entidade [managedAppRegistration](/graph/api/resources/intune-mam-managedappregistration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **enabledForScopeValidation** à entidade [resourceOperation](/graph/api/resources/intune-rbac-resourceoperation?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **claimTokenManagementFromExternalMdm** à entidade [vppToken](/graph/api/resources/intune-onboarding-vpptoken?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **windows10AppsForceUpdateSchedule** à entidade [windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **deploymentProfileAssignmentDetailedStatus** à entidade [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades de navegação **deviceConfigurationConflictSummary** e **importedWindowsAutopilotDeviceIdentityUploads** à entidade [deviceManagement](/graph/api/resources/intune-shared-devicemanagement?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade de navegação **intendedDeploymentProfile** à entidade [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **startLayoutTileSize** e o **nome** para o tipo de [windowsKioskAppBase](/graph/api/resources/intune-deviceconfig-windowskioskappbase?view=graph-rest-beta) complexo|
|Adição|beta|Adicionadas as propriedades **desktopApplicationId** e **desktopApplicationLinkPath** ao tipo complexo [windowsKioskDesktopApp](/graph/api/resources/intune-deviceconfig-windowskioskdesktopapp?view=graph-rest-beta)|
|Exclusão|beta|Removida a propriedade **name** do tipo complexa [windowsKioskDesktopApp](/graph/api/resources/intune-deviceconfig-windowskioskdesktopapp?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **disallowDesktopApps** para o tipo de [windowsKioskMultipleApps](/graph/api/resources/intune-deviceconfig-windowskioskmultipleapps?view=graph-rest-beta) complexo|
|Alteração|beta|Alteradas as seguintes propriedades no tipo complexo [windowsKioskMultipleApps](/graph/api/resources/intune-deviceconfig-windowskioskmultipleapps?view=graph-rest-beta) :<br/>**startMenuLayoutXml** de necessária para opcional<br/>|
|Adição|beta|Adicionadas as propriedades **v10_1607**, **v10_1703**, **v10_1709** e **v10_1803** ao tipo complexo [windowsMinimumOperatingSystem](/graph/api/resources/intune-apps-windowsminimumoperatingsystem?view=graph-rest-beta)|
|Adição|beta|Adicionado o membro **paloAltoGlobalProtect** para o tipo de enum [androidWorkProfileVpnConnectionType](/graph/api/resources/intune-deviceconfig-androidworkprofilevpnconnectiontype?view=graph-rest-beta)|
|Adição|beta|Adicionado o membro **remoteLock** para o tipo de enum [deviceComplianceActionType](/graph/api/resources/intune-deviceconfig-devicecomplianceactiontype?view=graph-rest-beta)|
...
| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Adição|beta|API adicionada para [guias](/graph/api/resources/teamstab?view=graph-rest-beta).|
|Adição|beta|API adicionada para [publicação de aplicativos para sua organização](/graph/api/resources/teamsapp?view=graph-rest-beta).|
|Adição|beta|Permissões de aplicativo adicionado suportam para [obter /teams/ {id}](/graph/api/team-get?view=graph-rest-beta). |
|Adição|beta|Adicionado o suportam de permissões do aplicativo para [GET /teams/ {id} / canais](/graph/api/group-list-channels?view=graph-rest-beta). |
|Adição|beta|Permissões de aplicativo adicionado suportam para [obter /channels/ /teams/ {id} {id}](/graph/api/channel-get?view=graph-rest-beta). |
|Adição|beta|Permissões de aplicativo adicionado suportam para [colocar /groups/ {id} / equipe](/graph/api/team-put-teams?view=graph-rest-beta). |
|Adição|beta|Adicionado o suporte de permissões do aplicativo a [corrigir /teams/ {id}](/graph/api/team-update?view=graph-rest-beta). |
|Adição|beta|Adicionado suportam de permissões de aplicativo ao [criar canal](/graph/api/channel-post?view=graph-rest-beta), [o canal de atualização](/graph/api/channel-patch?view=graph-rest-beta)e [Excluir canal](/graph/api/channel-delete?view=graph-rest-beta). |
|Exclusão|beta| Propriedades isBlocks e installedState removidas do [teamsApp](/graph/api/resources/teamsapp?view=graph-rest-beta).|
|Alteração| beta | A propriedade context em [teamsApp](/graph/api/resources/teamsapp?view=graph-rest-beta) foi renomeada para distributionMethod.|

### <a name="outlook-mail"></a>Email do Outlook

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição        | v1.0 e beta | A propriedade **internetMessageHeaders** da entidade a [mensagem](/graph/api/resources/message?view=graph-rest-1.0) agora é gravável na criação de mensagem. |


### <a name="project-rome-notifications-api"></a>Notificações de Roma API do Project

| **Tipo de alteração** | **Versão** | **Descrição**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Adição          | Beta        | Adicionado o tipo de recurso de [notificação](/graph/api/resources/projectrome-notification?view=graph-rest-beta) . |
| Adição          | Beta        | Adicionado o [criar e publicar notificação] (/ gráfico/api/projectrome_notification_post? view = gráfico-rest-beta) API.|

### <a name="security-apis"></a>APIs de segurança

| **Tipo de alteração** | **Versão** | **Descrição**              |
| :-------------- | :---------- | :--------------------------------------- |
| Adição        | Beta       | Adicionadas as APIs de pontuação seguro para a [API de segurança](/graph/api/resources/securescore-api-overview?view=graph-rest-beta), incluindo as operações e os recursos a seguir:<br/>[secureScores](/graph/api/resources/securescores?view=graph-rest-beta) (e entidades relacionadas)<br/>[Lista secureScores](/graph/api/securescores-list?view=graph-rest-beta)<br/>[secureScoreControlProfiles](/graph/api/resources/securescorecontrolprofiles?view=graph-rest-beta)<br/>[Lista secureScoreControlProfiles](/graph/api/securescorecontrolprofiles-list?view=graph-rest-beta)<br/>[Atualizar secureScoreControlProfiles](/graph/api/securescorecontrolprofiles-update?view=graph-rest-beta)


### <a name="onedrive-and-sharepoint-apis"></a>OneDrive e APIs do SharePoint

| **Tipo de alteração** | **Versão** | **Descrição**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Adição        | Beta        | Adicionado o argumento **deferCommit** para a ação de [createUploadSession](/graph/api/driveitem-createuploadsession?view=graph-rest-beta) em [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta)|
| Adição        | Beta        | Adicionado o tipo de [storagePlanInformation](/graph/api/resources/storageplaninformation?view=graph-rest-beta) complexo |
| Adição        | Beta        | Adicionada a propriedade **storagePlanInformation** para o tipo complexo de [cota](/graph/api/resources/quota?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a propriedade de navegação a **seguir** para a entidade de [unidade](/graph/api/resources/drive?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a ação que [siga](/graph/api/driveitem-follow?view=graph-rest-beta) em [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) |
| Adição        | Beta        | Adicionado o [deixar de seguir](/graph/api/driveitem-unfollow?view=graph-rest-beta) API |
| Adição        | Beta        | Adicionada a propriedade **hasPassword** à entidade de [permissão](/graph/api/resources/permission?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a propriedade **preventsDownload** para o tipo de [sharingLink](/graph/api/resources/sharinglink?view=graph-rest-beta) complexo |
| Adição        | Beta        | Adicionada a propriedade de navegação **permission** à entidade [sharedDriveItem](/graph/api/resources/shareddriveitem?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a propriedade **localização geográfica** à entidade [columnDefinition](/graph/api/resources/columndefinition?view=graph-rest-beta) |
| Adição        | Beta        | Adicionado o tipo de [geolocationColumn](/graph/api/resources/geolocationcolumn?view=graph-rest-beta) complexo |
| Adição        | Beta        | Adicionada a propriedade **analytics** à entidade [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a propriedade de **análise** para a entidade do [site](/graph/api/resources/site?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a propriedade **analytics** à entidade [listItem](/graph/api/resources/listitem?view=graph-rest-beta) |
...
| Adição        | Beta        | Adicionada a função **getActivitiesByInterval** sobre a entidade do [site](/graph/api/resources/site?view=graph-rest-beta) |
...
| Adição        | Beta        | Adicionado o tipo de [itemActionStat](/graph/api/resources/itemactionstat?view=graph-rest-beta) complexo |
| Adição        | Beta        | Adicionado o tipo de [accessAction](/graph/api/resources/accessaction?view=graph-rest-beta) complexo |
| Adição        | Beta        | Adicionado o tipo de [incompleteData](/graph/api/resources/incompletedata?view=graph-rest-beta) complexo |
| Adição        | Beta        | Adicionada a propriedade **acesso** ao tipo complexo [itemActivity](/graph/api/resources/itemactivity?view=graph-rest-beta) |
| Adição        | Beta        | Adicionada a propriedade **location** para o tipo de [itemActivity](/graph/api/resources/itemactivity?view=graph-rest-beta) complexo |
| Adição        | v1.0        | Adicionada a ação de **visualização** na entidade [driveItem](/graph/api/resources/driveitem?view=graph-rest-1.0) |
| Adição        | v1.0        | Adicionado o tipo de [itemPreviewInfo](/graph/api/resources/itempreviewinfo?view=graph-rest-1.0) complexo |
...
| Adição | v1.0 | Propriedade adicionado [onPremisesProvisioningErrors](/graph/api/resources/onpremisesprovisioningerror?view=graph-rest-1.0) para as entidades de [usuário](/graph/api/resources/user?view=graph-rest-1.0) e [grupo](/graph/api/resources/group?view=graph-rest-1.0) que representa os erros de sincronização de diretório durante a sincronização de diretórios no local com o Windows Azure Active Directory ao usar o Microsoft produto de sincronização (incluindo Connect do Azure AD, DirSync e MIM + conector).|
| Adição | v1.0 | Adicionado [onPremisesExtensionAttributes](/graph/api/resources/onpremisesextensionattributes?view=graph-rest-1.0) propriedade para a entidade de [usuário](/graph/api/resources/user?view=graph-rest-1.0) que contém as propriedades de atributo quinze extensão personalizada. Um usuário onPremisesSyncEnabled, esse conjunto de propriedades é administrado no Active Directory no local e sincronizado com o Azure AD e é somente leitura. Para um usuário somente na nuvem (onde onPremisesSyncEnabled é false), essas propriedades podem ser definida durante a criação ou atualização.|
|Adição|v1.0|Adicionadas as propriedades **onPremisesDomainName**, **onPremisesSamAccountName**e **onPremisesUserPrincipalName** à entidade de [usuário](/graph/api/resources/user?view=graph-rest-1.0)|
...
|Adição|v1.0|Adicionada a propriedade **kioskModeBuiltInAppId** à entidade [iosGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-iosgeneraldeviceconfiguration?view=graph-rest-1.0)|
|Adição|v1.0|Adicionado o membro **não atribuído** para o tipo de enum [complianceStatus](/graph/api/resources/intune-shared-compliancestatus?view=graph-rest-1.0)|
|Adição|v1.0|Adicionado o membro **pushNotification** para o tipo de enum [deviceComplianceActionType](/graph/api/resources/intune-deviceconfig-devicecomplianceactiontype?view=graph-rest-1.0)|
|Adição|v1.0|Adicionado o membro **userAbandonment** para o tipo de enum [deviceEnrollmentFailureReason](/graph/api/resources/intune-troubleshooting-deviceenrollmentfailurereason?view=graph-rest-1.0)|
|Adição|v1.0|Adicionados os membros **comprometido** e **configurado incorretamente** no tipo de enum [managedDevicePartnerReportedHealthState](/graph/api/resources/intune-devices-manageddevicepartnerreportedhealthstate?view=graph-rest-1.0)|
|Adição|v1.0|Adicionado o membro **assignedToExternalMDM** para o tipo de enum [vppTokenState](/graph/api/resources/intune-onboarding-vpptokenstate?view=graph-rest-1.0)|
||
...
|Adição|beta|Adicionada a ação [uploadDepToken](/graph/api/intune-enrollment-deponboardingsetting-uploaddeptoken?view=graph-rest-beta) em [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação [syncWithAppleDeviceEnrollmentProgram](/graph/api/intune-enrollment-deponboardingsetting-syncwithappledeviceenrollmentprogram?view=graph-rest-beta) em [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação [setDefaultProfile](/graph/api/intune-enrollment-enrollmentprofile-setdefaultprofile?view=graph-rest-beta) em [enrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação **importAppleDeviceIdentityList** no conjunto de [importedAppleDeviceIdentity](/graph/api/resources/intune-enrollment-importedappledeviceidentity?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação [updateDeviceProfileAssignment](/graph/api/intune-enrollment-enrollmentprofile-updatedeviceprofileassignment?view=graph-rest-beta) em [enrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação [shareForSchoolDataSyncService](/graph/api/intune-enrollment-deponboardingsetting-shareforschooldatasyncservice?view=graph-rest-beta) em [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação [unshareForSchoolDataSyncService](/graph/api/intune-enrollment-deponboardingsetting-unshareforschooldatasyncservice?view=graph-rest-beta) em [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Adição|beta|Adicionada a ação [assignUserToDevice](/graph/api/intune-enrollment-windowsautopilotdeviceidentity-assignusertodevice?view=graph-rest-beta) em [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta) |
|Adição|beta|Adicionada a função [getRoleScopeTagsByResource](/graph/api/intune-rbac-devicemanagement-getrolescopetagsbyresource?view=graph-rest-beta) em [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) |
...
|Adição|beta|Adicionada a função [getEncryptionPublicKey](/graph/api/intune-enrollment-deponboardingsetting-getencryptionpublickey?view=graph-rest-beta) em [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Adição|beta|Adicionada a função [exportMobileConfig](/graph/api/intune-enrollment-enrollmentprofile-exportmobileconfig?view=graph-rest-beta) em [enrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta) |
|Adição|beta|Adicionada a função [autopilotDeviceStream](/graph/api/intune-enrollment-importedwindowsautopilotdeviceidentityupload-autopilotdevicestream?view=graph-rest-beta) em [importedWindowsAutopilotDeviceIdentityUpload](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityupload?view=graph-rest-beta) |
|Exclusão|beta|Removida a coleção **uploadDepToken** |
|Exclusão|beta|Removida a ação **syncWithAppleDeviceEnrollmentProgram** no conjunto de [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Exclusão|beta|Removida a função **getEncryptionPublicKey** no conjunto de [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Adição|beta|Adicionada a propriedade **restrictedApps** à entidade [androidForWorkCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidforworkcompliancepolicy?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **vpnAlwaysOnPackageIdentifier** e **vpnEnableAlwaysOnLockdownMode** à entidade [androidForWorkGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidforworkgeneraldeviceconfiguration?view=graph-rest-beta)|
|Exclusão|beta|Removida a propriedade **encapsulamento** da entidade [androidForWorkMobileAppConfiguration](/graph/api/resources/intune-apps-androidforworkmobileappconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **restrictedApps** à entidade [androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **vpnAlwaysOnPackageIdentifier** e **vpnEnableAlwaysOnLockdownMode** à entidade [androidWorkProfileGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidworkprofilegeneraldeviceconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **optInToDeviceIdSharing** à entidade [appleVpnConfiguration](/graph/api/resources/intune-deviceconfig-applevpnconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **tokenType**, **tokenName**, **syncedDeviceCount**, **defaultProfileDisplayName** e **dataSharingConsentGranted** à entidade [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **roleScopeTagIds** à entidade [deviceCompliancePolicy](/graph/api/resources/intune-deviceconfig-devicecompliancepolicy?view=graph-rest-beta)|
...
|Adição|beta|Adicionada a propriedade **windowsMalwareOverview** à entidade [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)|
|Alteração|beta|Alteradas as seguintes propriedades na entidade [iosCertificateProfileBase](/graph/api/resources/intune-deviceconfig-ioscertificateprofilebase?view=graph-rest-beta) :<br/>**subjectAlternativeNameType** de obrigatória para opcional<br/>|
...
|Adição|beta|Adicionadas as propriedades **certificateStore** e **customSubjectAlternativeNames** à entidade [iosScepCertificateProfile](/graph/api/resources/intune-deviceconfig-iosscepcertificateprofile?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **enforcedSoftwareUpdateDelayInDays** à entidade [iosUpdateConfiguration](/graph/api/resources/intune-deviceconfig-iosupdateconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **providerType**, **userDomain**, **strictEnforcement**, **cloudName** e **excludeList** à entidade [iosVpnConfiguration](/graph/api/resources/intune-deviceconfig-iosvpnconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionado o **safariBlockAutofill**, **cameraBlocked**, **iTunesBlockMusicService**, **spotlightBlockInternetResults**, **keyboardBlockDictation**, **definitionLookupBlocked**, ** appleWatchBlockAutoUnlock**, **iTunesBlockFileSharing**, **iCloudBlockDocumentSync**, **iCloudBlockMail**, **iCloudBlockAddressBook**, **iCloudBlockCalendar**, **iCloudBlockReminders**, ** iCloudBlockBookmarks**, propriedades **iCloudBlockNotes**, **airDropBlocked**, **passwordBlockModification** e **passwordBlockFingerprintUnlock** à entidade [macOSGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-macosgeneraldeviceconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **roleScopeTagIds**, **windowsActiveMalwareCount**, **windowsRemediatedMalwareCount**, **anotações** e **configurationManagerClientHealthState** à entidade [managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **installStateDetail** à entidade [mobileAppInstallStatus](/graph/api/resources/intune-apps-mobileappinstallstatus?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **roleScopeTagIds** à entidade [notificationMessageTemplate](/graph/api/resources/intune-notification-notificationmessagetemplate?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **targetVersion** e **updateVersion** à entidade [officeSuiteApp](/graph/api/resources/intune-apps-officesuiteapp?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade de **recurso** à entidade [resourceOperation](/graph/api/resources/intune-rbac-resourceoperation?view=graph-rest-beta)|
...
|Adição|beta|Adicionada a propriedade **configurationManagerComplianceRequired** à entidade [windows10CompliancePolicy](/graph/api/resources/intune-deviceconfig-windows10compliancepolicy?view=graph-rest-beta)|
|Adição|beta|Adicionado o **userRightsAccessCredentialManagerAsTrustedCaller**, **userRightsAllowAccessFromNetwork**, **userRightsBlockAccessFromNetwork**, **userRightsActAsPartOfTheOperatingSystem**, **userRightsLocalLogOn **, **userRightsBackupData**, **userRightsChangeSystemTime**, **userRightsCreateGlobalObjects**, **userRightsCreatePageFile**, **userRightsCreatePermanentSharedObjects**, ** userRightsCreateSymbolicLinks**, **userRightsCreateToken**, **userRightsDebugPrograms**, **userRightsRemoteDesktopServicesLogOn**, **userRightsDelegation**, **userRightsGenerateSecurityAudits **, **userRightsImpersonateClient**, **userRightsIncreaseSchedulingPriority**, **userRightsLoadUnloadDrivers**, **userRightsLockMemory**, **userRightsManageAuditingAndSecurityLogs**, ** userRightsManageVolumes**, **userRightsModifyFirmwareEnvironment**, **userRightsModifyObjectLabels**, **userRightsProfileSingleProcess**, **userRightsRemoteShutdown**, **userRightsRestoreData **, **userRightsTakeOwnership**, **userRightsRegisterProcessAsService**, **localSecurityOptionsMinimumSessionSecurityForNtlmSspBasedClients**, ** localSecurityOptionsMinimumSessionSecurityForNtlmSspBasedServers**, propriedades **lanManagerAuthenticationLevel** e **lanManagerWorkstationEnableInsecureGuestLogons** para o [ windows10EndpointProtectionConfiguration](/graph/api/resources/intune-deviceconfig-windows10endpointprotectionconfiguration?view=graph-rest-beta) entidade|
|Adição|beta|Adicionadas as propriedades **passwordMinimumAgeInDays**, **tenantLockdownRequireNetworkDuringOutOfBoxExperience** e **dataProtectionBlockDirectMemoryAccess** à entidade [windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **extendedKeyUsages** à entidade [windows10PkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-windows10pkcscertificateprofile?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **enableDnsRegistration** e **dnsSuffixes** à entidade [windows10VpnConfiguration](/graph/api/resources/intune-deviceconfig-windows10vpnconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **customSubjectAlternativeNames** à entidade [windows81CertificateProfileBase](/graph/api/resources/intune-deviceconfig-windows81certificateprofilebase?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **extractHardwareHash** e **deviceNameTemplate** à entidade [windowsAutopilotDeploymentProfile](/graph/api/resources/intune-enrollment-windowsautopilotdeploymentprofile?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades **addressableUserName** e **userPrincipalName** à entidade [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **threatState** à entidade [windowsDeviceMalwareState](/graph/api/resources/intune-devices-windowsdevicemalwarestate?view=graph-rest-beta)|
|Adição|beta|Adicionado o **qualityUpdatesPauseStartDateTime**, **featureUpdatesPauseStartDateTime**, **featureUpdatesRollbackWindowInDays**, **qualityUpdatesWillBeRolledBack**, **featureUpdatesWillBeRolledBack**, ** qualityUpdatesRollbackStartDateTime** e propriedades de **featureUpdatesRollbackStartDateTime** à entidade [windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade **trustedServerCertificateNames** à entidade [windowsWifiEnterpriseEAPConfiguration](/graph/api/resources/intune-deviceconfig-windowswifienterpriseeapconfiguration?view=graph-rest-beta)|
|Adição|beta|Adicionadas as propriedades de navegação **defaultIosEnrollmentProfile**, **defaultMacOsEnrollmentProfile**, **enrollmentProfiles** e **importedAppleDeviceIdentities** à entidade [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)|
|Adição|beta|Adicionada a propriedade de navegação **roleScopeTags** à entidade [deviceAndAppManagementRoleAssignment](/graph/api/resources/intune-rbac-deviceandappmanagementroleassignment?view=graph-rest-beta)|
...
|Adição|beta|Adicionada a propriedade **supportedDeviceTypes** para o tipo de [mobileAppIntentAndStateDetail](/graph/api/resources/intune-troubleshooting-mobileappintentandstatedetail?view=graph-rest-beta) complexo|
|Adição|beta|Adicionada a propriedade **hideEscapeLink** para o tipo de [outOfBoxExperienceSettings](/graph/api/resources/intune-enrollment-outofboxexperiencesettings?view=graph-rest-beta) complexo|
|Adição|beta|Adicionados os membros **zscalerPrivateAccess**, **f5Access2018**, **citrixSso** e **paloAltoGlobalProtectV2** ao tipo enum [appleVpnConnectionType](/graph/api/resources/intune-deviceconfig-applevpnconnectiontype?view=graph-rest-beta)|
|Adição|beta|Adicionado o membro **userAbandonment** para o tipo de enum [deviceEnrollmentFailureReason](/graph/api/resources/intune-troubleshooting-deviceenrollmentfailurereason?view=graph-rest-beta)|
|Adição|beta|Adicionado o membro **bloqueado** para o tipo de enum [enrollmentState](/graph/api/resources/intune-enrollment-enrollmentstate?view=graph-rest-beta)|
|Adição|beta|Adicionado o membro **microsoft365ManagedMdm** para o tipo de enum [managementAgentType](/graph/api/resources/intune-devices-managementagenttype?view=graph-rest-beta)|
|Adição|beta|Adicionado o membro **domainNameService** para o tipo de enum [subjectAlternativeNameType](/graph/api/resources/intune-deviceconfig-subjectalternativenametype?view=graph-rest-beta)|
|Adição|beta|Adicionados os membros **wpa2Personal** e **wpa2Enterprise** ao tipo enum [wiFiSecurityType](/graph/api/resources/intune-deviceconfig-wifisecuritytype?view=graph-rest-beta)|
|Adição|beta|Adicionados os membros **enterpriseUnwantedSoftware**, **resgate** e **hipsRule** ao tipo enum [windowsMalwareCategory](/graph/api/resources/intune-devices-windowsmalwarecategory?view=graph-rest-beta)|

### <a name="outlook-calendar"></a>Calendário do Outlook

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição | Beta | Adicionado os [getSchedule](/graph/api/calendar-getschedule?view=graph-rest-beta) ação e o [freeBusyError](/graph/api/resources/freebusyerror?view=graph-rest-beta), [scheduleInformation](/graph/api/resources/scheduleinformation?view=graph-rest-beta)e [scheduleItem](/graph/api/resources/scheduleitem?view=graph-rest-beta) tipos complexos para oferecer suporte a [Obtendo as informações de disponibilidade, informações de disponibilidade para usuários, listas de distribuição e recursos para um determinado período de tempo](outlook-get-free-busy-schedule.md). |

### <a name="outlook-mail"></a>Email do Outlook

| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição        | v1.0        | Adicionado suporte para a ação [getMailTips](/graph/api/user-getmailtips?view=graph-rest-1.0) obter qualquer dicas de email para destinatários específicos. Adicionados os seguintes recursos: [automaticRepliesMailTips](/graph/api/resources/automaticrepliesmailtips?view=graph-rest-1.0), [dicas de email](/graph/api/resources/mailtips?view=graph-rest-1.0), [mailTipsError](/graph/api/resources/mailtipserror?view=graph-rest-1.0). |
...
| Adição    | v1.0    | Adicionada a propriedade de **Ativação do computador compartilhado** para [getoffice365activationsusercounts](/graph/api/reportroot-getoffice365activationsusercounts?view=graph-rest-1.0). |

### <a name="security-apis"></a>APIs de segurança

| **Tipo de alteração** | **Versão** | **Descrição**              |
| :-------------- | :---------- | :--------------------------------------- |
| Adição        | beta       | Adicionado **activityGroupName**, **cloudAppStates**, **confiança**e propriedades **registryKeyStates** para [alerta](/graph/api/resources/alert?view=graph-rest-beta ). |
|Exclusão|beta| Removidos propriedades **activityGroupStates**, **applicationStates**, **malwareWasRunning**, **riskScore** e **tipo** de [alerta](/graph/api/resources/alert?view=graph-rest-beta ). |
|Alteração|beta| Alterado o tipo de **comentários** de um `String` para um `String collection`e alterado o tipo de **gravidade** de um `String` para um enum [alertSeverity](/graph/api/resources/alertseverityenumtype?view=graph-rest-beta) no [alerta](/graph/api/resources/alert?view=graph-rest-beta). |
| Adição        | beta       | Adicionados os seguintes tipos de recursos: <br/> [cloudAppSecurityState](/graph/api/resources/cloudappsecuritystate?view=graph-rest-beta) <br/> [fileHash](/graph/api/resources/filehash?view=graph-rest-beta) <br/> [registryKeyState](/graph/api/resources/registrykeystate?view=graph-rest-beta) |
|Exclusão|beta| Removidos os seguintes tipos de recursos: <br/> **activityGroupState**  <br/> **applicationSecurityState** |
| Adição        | beta       | Adicionados as enumerações a seguintes: <br/> [alertSeverity](/graph/api/resources/alertseverityenumtype?view=graph-rest-beta) <br/> [connectionDirection](/graph/api/resources/connectiondirectionenumtype?view=graph-rest-beta) <br/> [connectionStatus](/graph/api/resources/connectionstatusenumtype?view=graph-rest-beta) <br/> [emailRole](/graph/api/resources/emailroleenumtype?view=graph-rest-beta) <br/> [fileHashType](/graph/api/resources/filehashtypeenumtype?view=graph-rest-beta) <br/> [registryHive](/graph/api/resources/registryhiveenumtype?view=graph-rest-beta)  <br/> [registryOperation](/graph/api/resources/registryoperationenumtype?view=graph-rest-beta) <br/> [registryValueType](/graph/api/resources/registryvaluetypeenumtype?view=graph-rest-beta)|
|Exclusão|Beta| Foram removidos os seguintes tipos de enumeração: <br/> **alertType** <br/> **applicationPermissionsRequired** |
| Adição        | beta       | Adicionada a propriedade **fileHash** à [fileSecurityState](/graph/api/resources/filesecuritystate?view=graph-rest-beta ).|
|Exclusão|beta| Propriedades **authenticodeHash256** e **sha256** removidas do [fileSecurityState](/graph/api/resources/filesecuritystate?view=graph-rest-beta). |
| Adição | beta | Adicionada a propriedade **SO** à [hostSecurityState](/graph/api/resources/hostsecuritystate?view=graph-rest-beta).|
| Adição | beta | Propriedades de **categoria**, **familiares**e **wasRunning** adicionadas para [malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta).|
|Exclusão|beta| Removida a propriedade **aliases** do [malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta). |
|Alteração|beta| Movido propriedade **malwareWasRunning** de [alerta](/graph/api/resources/alert?view=graph-rest-beta ) para [malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta) e renomeado para **wasRunning**. |
| Adição        | beta       | Adicionado **applicationName**, **destinationDomain**, **direção**, **domainRegisteredDateTime**, **localDnsName**, **natDestinationAddress**, **natDestinationPort**, **natSourceAddress**, ** natSourcePort**, **riskScore**, **status**e **urlParameters** propriedades para [networkConnection](/graph/api/resources/networkconnection?view=graph-rest-beta ).|
|Alteração|beta| Alterado a propriedade **uri** para **destinationUrl** em [networkConnection](/graph/api/resources/networkconnection?view=graph-rest-beta ). |
| Adição        | beta       | Adicionada a propriedade **fileHash** ao [processo](/graph/api/resources/process?view=graph-rest-beta ).|
|Exclusão|beta| Propriedades **authenticodeHash256** e **sha256** removidas do [processo](/graph/api/resources/process?view=graph-rest-beta ). |
| Adição        | beta       | Adicionado **aadUserId**, **emailRole**, **isVpn**e propriedades de **logonIp** para [userSecurityState](/graph/api/resources/usersecuritystate?view=graph-rest-beta).|
|Alteração|beta| Alterado a propriedade **logonIpAddress** para **logonIp** em [userSecurityState](/graph/api/resources/usersecuritystate?view=graph-rest-beta). |
| Adição        | beta       | Adicionada a propriedade **wasRunning** à [vulnerabilityState](/graph/api/resources/vulnerabilitystate?view=graph-rest-beta).|
|Exclusão|beta| Removida a propriedade **name** do [vulnerabilityState](/graph/api/resources/vulnerabilitystate?view=graph-rest-beta). |
...
| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Alteração|beta|Recurso atualizados [chatmessage](/graph/api/resources/chatmessage?view=graph-rest-beta)|
...
| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição | Beta | Adicionada a propriedade **progress** ao [sychronizationStatus](/graph/api/resources/synchronization-synchronizationstatus?view=graph-rest-beta) para permitir que clientes para monitorar o progresso de um trabalho de sincronização.|
...
|Alteração | Beta | Alterado o tipo da propriedade **emailAddresses** de [contact](/graph/api/resources/contact?view=graph-rest-beta) para ser um conjunto de instâncias **typedEmailAddress**.|
...
| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição | beta | Adicionado o recurso [acesso analisa](/graph/api/resources/accessreviews-root?view=graph-rest-beta) ao [Azure AD](/graph/api/resources/azure-ad-overview?view=graph-rest-beta). |
...
| Adição | v1.0 | Adicionado **ageGroup**, **legalAgeGroupClassification**e **ConsentRequiredForMinor** propriedades para o recurso de [usuário](/graph/api/resources/user?view=graph-rest-1.0)
...
| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Adição        | v1.0 e beta | Foi adicionada a propriedade **importance** à entidade [post](/graph/api/resources/post?view=graph-rest-1.0). |
...
|Exclusão|beta|Removida a função exportMobileConfig](/graph/api/intune_corpenrollment_enrollmentprofile_exportmobileconfig?view=graph-rest-beta) em [enrollmentProfile](/graph/api/resources/intune-corpenrollment-enrollmentprofile?view=graph-rest-beta) |
...
| **Tipo de alteração** | **Versão**   | **Descrição**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Alteração           | Beta          | A propriedade **creatorUserId** da entidade [subscription](/graph/api/resources/subscription?view=graph-rest-beta) foi renomeada para **creatorId** para refletir melhor o significado. |
...
| Alteração | v1.0 | A propriedade renewedDateTime e [renew](/graph/api/group-renew?view=graph-rest-1.0) foi adicionada a [group](/graph/api/resources/group?view=graph-rest-1.0) |
...
|Adição|Beta|Foi adicionada a ação [preview](/graph/api/driveitem-preview?view=graph-rest-beta) à entidade [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) |
...
|Alteração|Beta|O limite interno de solicitações de [processamento em lotes JSON](json-batching.md) foi aumentado de 5 para 20.|

### <a name="education-apis"></a>APIs de educação

|Tipo de alteração|Versão|Descrição|
|:---|:---|:---|
|Adição|Beta|Acrescente propriedades de navegação adicionais e melhore o suporte a filtragem para a [API da lista de participação](/graph/api/resources/education-overview?view=graph-rest-beta).|
...
| Adição        | Beta        | Adicionado suporte para a ação [getMailTips](/graph/api/user-getmailtips?view=graph-rest-beta) obter qualquer dicas de email para destinatários específicos. Adicionados os seguintes recursos: [automaticRepliesMailTips](/graph/api/resources/automaticrepliesmailtips?view=graph-rest-beta), [dicas de email](/graph/api/resources/mailtips?view=graph-rest-beta), [mailTipsError](/graph/api/resources/mailtipserror?view=graph-rest-beta). |
...
|**Tipo de alteração**|**Versão**|**Descrição**|
|:--------------|:-----------|:--------------|
|Alteração significativa|Beta|Alterações do findMeetingTimes API. Para obter mais informações, consulte a postagem do blog do [API do Microsoft Graph findMeetingTimes atualizar](https://dev.office.com/microsoft-graph-findmeetingtimes-api-update) . Essa alteração está em vigor em 19 de maio de 2016.
