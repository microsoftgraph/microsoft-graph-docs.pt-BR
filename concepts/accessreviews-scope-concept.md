...
A Azure AD [api](/graph/api/resources/accessreviewsv2-overview) de revisões de acesso permite que você examine programaticamente o acesso que os usuários, entidades de serviço ou grupos têm aos seus Azure AD recursos. A API pode ajudá-lo a automatizar a revisão proativa e manter o controle sobre o acesso aos recursos em sua organização.  
...
|[principalResourceMembershipsScope](/graph/api/resources/principalResourceMembershipsScope)|Herda de **accessReviewScope**. Melhor aplicável para examinar o acesso das entidades de segurança aos recursos em que você configura pools exclusivos de entidades de segurança e recursos.|<ul><li>Revisando o acesso de três entidades de segurança específicas em um grupo do Microsoft 365 e *uma* função Azure AD privilegiada.</li><ul>|
...
### <a name="example-4-review-all-users-assigned-to-all-microsoft-365-groups"></a>Exemplo 4: Examinar todos os usuários atribuídos a todos os grupos do Microsoft 365

O exemplo a seguir cria uma revisão de cada grupo do Microsoft 365 que contém usuários convidados. Para a instância de revisão em cada grupo, a revisão tem como escopo apenas membros diretos do grupo que são usuários convidados.
...
Além disso, como essa revisão é aplicada a todos os grupos do Microsoft 365, configure **o instanceEnumerationScope** para especificar os grupos do Microsoft 365 a serem examinados. Grupos dinâmicos e grupos atribuíveis a funções não estão incluídos nesta revisão.

### <a name="example-5-review-all-guest-users-assigned-to-all-microsoft-365-groups"></a>Exemplo 5: Examinar todos os usuários convidados atribuídos a todos os grupos do Microsoft 365

O exemplo a seguir tem como escopo a revisão para membros diretos de todos os grupos do Microsoft 365 que são usuários convidados.
...
#### <a name="review-all-inactive-guest-users-assigned-to-all-microsoft-365-groups"></a>Examinar todos os usuários convidados inativos atribuídos a todos os grupos do Microsoft 365

O exemplo a seguir tem como escopo a revisão para membros diretos de todos os Microsoft 365 que são usuários convidados inativos.
...
Além disso, como essa revisão é aplicada a todos os grupos do Microsoft 365 habilitados para o Teams, configure **o instanceEnumerationScope** para especificar os grupos do Microsoft 365 habilitados para Teams a serem examinados. Grupos dinâmicos e grupos atribuíveis a funções não estão incluídos nesta revisão.
...
Para examinar usuários e equipes do B2B Direct Connect em canais compartilhados, `/teams/{groupId}/channels?$filter=(membershipType eq 'shared')` você deve especificar o padrão de consulta no **objeto resourceScopes**. Uma *revisão de todas* as equipes, como o [Exemplo 6](#review-all-inactive-guest-users-assigned-to-all-teams), não incluirá usuários de conexão direta B2B e equipes em canais compartilhados.
