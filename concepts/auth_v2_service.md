...
Com o fluxo de Concessão de Credenciais do Cliente OAuth 2.0, seu aplicativo autentica diretamente no ponto de extremidade `/token` do Azure AD v2.0 usando a Id de aplicativo atribuída pelo Azure AD e o Segredo do Aplicativo que você cria usando o portal. 

## <a name="2-configure-permissions-for-microsoft-graph"></a>2. Configure permissões para o Microsoft Graph
Para aplicativos que são executados sem um usuário, o Microsoft Graph expõe Permissões de aplicativo. Ao registrar seu aplicativo, configure previamente essas permissões. As Permissões de aplicativo sempre exigem o consentimento do administrador. Um administrador pode autorizar essas permissões usando o [Portal do Azure](https://portal.azure.com) quando o aplicativo for instalado em sua organização ou pode fornecer uma experiência de inscrição em seu aplicativo por meio da qual os administradores podem concordar com as permissões configuradas. Depois que o consentimento do administrador for registrado pelo Azure AD, seu aplicativo poderá solicitar tokens sem ter que solicitar consentimento novamente. Para obter mais informações sobre as permissões disponibilizadas pelo Microsoft Graph, confira a [Referência de permissões](./permissions_reference.md)

Para configurar as permissões do aplicativo para seu aplicativo no [Portal de Registro de Aplicativos da Microsoft](https://apps.dev.microsoft.com/): em **Microsoft Graph**, escolha **Adicionar** ao lado de **Permissões de Aplicativos** e, em seguida, escolha as permissões exigidas pelo aplicativo na caixa de diálogo **Selecionar Permissões**.

A captura de tela a seguir mostra a caixa d diálogo **Escolher Permissões** para permissões do Aplicativo do Microsoft Graph. 
...
O Azure AD exige que apenas um administrador do locatário possa fazer logon para concluir o pedido. O administrador será solicitado a aprovar todas as permissões de aplicativos diretos que você solicitou para seu aplicativo no portal de registro de aplicativos. Veja a seguir um exemplo de diálogo de consentimento que o Azure AD apresenta ao administrador:
...
No fluxo de Concessão de Credenciais do Cliente OAuth 2.0, você usa os valores da Id do aplicativo e o Segredo do Aplicativo salvos ao registrar seu aplicativo para solicitar um token de acesso diretamente ponto de extremidade `/token` do Azure AD v2.0.
...
Os serviços em segundo plano são executados em um servidor sem a presença de um usuário conectado e usam a Concessão de Credenciais do Cliente do OAuth 2.0 para autenticar no Azure AD e obter um token. Para o ponto de extremidade do v2.0, você pode explorar esse cenário ainda mais com os seguintes recursos:

- Para um tratamento mais completo do fluxo de Concessão de Credenciais do Cliente que também inclui respostas de erro, confira o [Azure Active Directory v2.0 e o fluxo de credenciais do cliente do OAuth 2.0](https://docs.microsoft.com/azure/active-directory/develop/active-directory-v2-protocols-oauth-client-creds). 
