...
description: Os provedores Graph Toolkit Microsoft permitem autenticação e acesso Graph Microsoft para todos os componentes.
...
|[Teams MSAL2](./teams-msal2.md)|Usa o msal-browser para entrar em usuários e adquirir tokens Microsoft Teams guias. Oferece suporte a Sign-On com back-back-back personalizado. |
...
Inicializar seu provedor em seu código JavaScript permite que você forneça mais opções. Para fazer isso, crie uma nova instância de provedor e de `Providers.globalProvider` definir o valor da propriedade para o provedor que você gostaria de usar. O exemplo a seguir mostra como usar o provedor MSAL2.
...
O provedor mantém o controle do estado de autenticação do usuário e o comunica aos componentes. Por exemplo, quando um usuário faz a entrada com êxito, `ProviderState` `SignedIn`o é atualizado para , sinalizando para os componentes que agora são capazes de fazer chamadas para o Microsoft Graph. O `ProviderState` número define três estados, conforme mostrado.
...
```
Você também pode usar o método `Providers.onProviderUpdated` para ser notificado sempre que o estado do provedor mudar.
...
Cada provedor expõe uma função chamada que `getAccessToken` pode recuperar o token de acesso atual ou recuperar um novo token de acesso para os escopos fornecidos. O exemplo a seguir mostra como obter um novo token de acesso com o escopo `User.Read` de permissão.
...
Quando você tem os provedores corretos inicializados para seu aplicativo, você pode adicionar o componente [de Logon](../components/login.md) do Toolkit para implementar de forma fácil e rápida o logout do usuário. O componente funciona com o provedor para lidar com toda a lógica de autenticação e a funcionalidade de logout/logout. O exemplo a seguir usa o provedor MSAL2 e o componente logon.
...
Em cenários em que você deseja implementar isso por conta própria, em vez de usar o componente de Logon do Toolkit, `login` `logout` você pode fazer isso usando os métodos e do provedor.

## <a name="implement-your-own-provider"></a>Implementar seu próprio provedor

Em cenários em que você deseja adicionar Toolkit componentes a um aplicativo com código de autenticação pré-existente, você pode criar um provedor personalizado que se conectar ao mecanismo de autenticação, em vez de usar nossos provedores predefinidos. O kit de ferramentas fornece duas maneiras de criar novos provedores:

- Crie um novo `SimpleProvider` que retorna um token de acesso do código de autenticação passando uma função.
- Estender a classe `IProvider` abstrata.

Para obter mais detalhes sobre cada um deles, consulte [provedores personalizados](../providers/custom.md).
