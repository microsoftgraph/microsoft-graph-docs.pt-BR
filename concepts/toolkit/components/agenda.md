...
| fuso horário preferido | preferredTimezone | Nome do fuso horário preferencial a ser usado ao recuperar eventos do Microsoft Graph; por exemplo, `Pacific Standard Time` . Por padrão, esse atributo usa o fuso horário UTC. O fuso horário preferencial para o usuário atual pode ser recuperado chamando-se o `me/mailboxSettings` ponto de extremidade e lendo o valor da propriedade **timeZone** . |
...
| Método | Descrição |
| --- | --- |
| reload () | Chame o método para recarregar o componente com possíveis novos dados com base em suas propriedades. |
...
O `mgt-agenda` componente oferece suporte a vários [modelos](../customize-components/templates.md) que permitem substituir determinadas partes do componente. Para especificar um modelo, inclua um `<template>` elemento dentro de um componente e defina o `data-type` valor como um dos seguintes:
