...
- **Use privilégios mínimos**. Apenas solicite permissões absolutamente necessárias e somente quando você precisar delas. Para as APIs que o aplicativo chama, verifique a seção de permissões nos tópicos do método (por exemplo, confira [como criar um usuário](../api-reference/v1.0/api/user_post_users.md) e escolha as permissões com menos privilégios. Uma ver uma lista completa de permissões, confira [referência de permissões](permissions_reference.md).
...
Use [notificações de webhook](../api-reference/v1.0/resources/webhooks.md) para receber notificações por push quando os dados forem alterados.
...
Use a [consulta delta](delta_query_overview.md) para manter os dados atualizados com eficiência.
...
Use as [notificações de webhook](../api-reference/v1.0/resources/webhooks.md) como o gatilho para fazer chamadas de consulta delta. Você também deve garantir que seu aplicativo tenha um limite de pesquisa de backstop, caso nenhuma notificação seja acionada.
...
Use o [envio em lote](json_batching.md) onde uma latência de rede significativa pode ter um grande impacto no desempenho.
...
- Sempre registre a *request-id* e o *carimbo de data/hora* no cabeçalho de resposta HTTP. Essa ação é necessária ao escalar ou relatar problemas no Stack Overflow ou no Atendimento ao Cliente da Microsoft.
