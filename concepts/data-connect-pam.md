...
Os pipelines do Data Connect devem ser aprovados por um membro de um grupo aprovador da solicitação de acesso aos dados. Aprovadores podem aprovar, recusar ou revogar pipelines usando a experiência do usuário PAM ou o módulo do PowerShell do Exchange Online.
...
4. Localizar todas as solicitações pendentes.
   >**Observação:** o valor nas propriedades de **identidade** será usada para identificar, aprovar ou negar a solicitação. Observe que esse valor será usado no parâmetro -RequestId. 

   ```powershell
   Get-ElevatedAccessRequest | ?{$_.RequestStatus -eq 'Pending'}
   ```

4. Veja mais detalhes no campo **contexto** da solicitação em que você está interessado. 
...
5. Aprovar/rejeitar a solicitação usando o valor de **identidade** para o parâmetro -RequestId.
...
Para revogar uma solicitação aprovada anteriormente, escolha a solicitação aprovada a ser revogada e clique em **Revogar**. Se a próxima tentativa de mover dados usar essa aprovação, ela irá falhar. 
