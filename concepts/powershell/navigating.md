...
Os comandos no SDK são gerados diretamente da [API REST](/graph/api/overview?view=graph-rest-1.0), de forma que os nomes são influenciados pela API. Você não precisa entender os detalhes da API para usar este SDK, mas ajuda a entender a Convenção de nomenclatura.
...
|-------------|--------------|---|
| OBTER         | Obter          | `Get-MgUser` [Referência da API](/graph/api/user-get?view=graph-rest-1.0) |
| POST        | Novo          | `New-MgUserMessage` [Referência da API](/graph/api/user-post-messages?view=graph-rest-1.0) |
| PUT         | Novo          | `New-MgTeam` [Referência da API](/graph/api/team-put-teams?view=graph-rest-1.0) |
| PATCH       | Atualizar       | `Update-MgUserEvent` [Referência da API](/graph/api/event-update?view=graph-rest-1.0) |
| DELETE      | Remover       | `Remove-MgDriveItem` [Referência da API](/graph/api/driveitem-delete?view=graph-rest-1.0) |
...
Vamos ver alguns exemplos. A API [getschedule](/graph/api/calendar-getschedule?view=graph-rest-1.0) usa `get` e `Get` é um verbo aprovado do PowerShell, portanto, é o comando `Get-MgUserCalendarSchedule` . A API de [cancelamento](/graph/api/event-cancel?view=graph-rest-beta) em um evento por outro lado, usa um verbo não aprovado `cancel` . O verbo aprovado para cancelamento ou descontinuação de algo é `Stop` , portanto, é o comando `Stop-MgUserEvent` . Por fim, o verbo da API [snoozeReminder](/graph/api/event-snoozereminder?view=graph-rest-1.0) , `snooze` , não tem nenhum equivalente aprovado no PowerShell. Para a API como essa, o SDK usa o verbo `Invoke` , para que o comando da API seja `Invoke-MgSnoozeUserEventReminder` .
...
Mas e os comandos como `Get-MgUserMessage` ou `Get-MgUserMailFolderMessage` ? Ambos são um objeto [Message](/graph/api/resources/message?view=graph-rest-1.0) , então por quê não `Get-MgMessage` ? A resposta vem da [API de mensagens Get](/graph/api/message-get?view=graph-rest-1.0).
