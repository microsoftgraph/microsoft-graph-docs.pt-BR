...
Seja no trabalho ou em uma instituição de ensino, procurar um horário e local em comum para uma reunião geralmente significa uma sobrecarga. Os aplicativos do Microsoft Graph podem usar a ação [findMeetingTimes](../api-reference/v1.0/api/user_findmeetingtimes.md) para identificar quaisquer possíveis horários de reunião que atendam aos requisitos de horário, local e outras restrições.   
...
As duas chamadas para **findMeetingTimes** incluem os seguintes parâmetros. Todos os [parâmetros](../api-reference/v1.0/api/user_findmeetingtimes.md#request-body) para **findMeetingTimes** são opcionais.

- **attendees**: um participante, Fanny Downs, definido como `required` para a propriedade **type**
