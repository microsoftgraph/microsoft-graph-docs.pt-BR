...
|:-------------|:------------|:------------|
|isDescending|Boolean|`True` para especificar a ordem de classificação como decrescente. O padrão é `false` , com a ordem de classificação como crescente. Opcional.|
|minimumCount|Int32|O número mínimo de itens que devem estar presentes na agregação a ser retornado em um bucket. Opcional.|
|prefixFilter|String|Um filtro para definir um critério correspondente. A chave deve começar com o prefixo especificado a ser retornado na resposta. Opcional.|
