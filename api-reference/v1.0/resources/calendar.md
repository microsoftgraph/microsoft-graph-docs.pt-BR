...
|[Listar eventos](../api/calendar_list_events.md) |Coleção [Event](event.md)| Recuperar uma lista de eventos em um calendário.  A lista contém reuniões de instância única e reuniões mestres em série.|
...
|changeKey|String|Identifica a versão do objeto calendar. Toda vez que o calendário é alterado, ChangeKey também muda. Isso permite que o Exchange aplique alterações na versão correta do objeto. Somente leitura.|
