...
|appsVisibilityListType|Cadeia de caracteres|Tipo de lista que está em AppsVisibilityList. Os valores possíveis são: `none`, `appsInListCompliant`, `appsNotInListCompliant`.|
...
|cellularBlockPerAppDataModification|Booliano|Indica se as alterações em configurações de uso de dados do aplicativo de rede celular serão ou não permitidas quando o dispositivo estiver no modo supervisionado (iOS 10.02 ou posterior).|
...
|deviceBlockEraseContentAndSettings|Booliano|Indica se será permitido ou não o uso da opção “Apagar todo o conteúdo e as configurações” quando o dispositivo estiver no modo supervisionado (iOS 10.02 ou posterior).|
...
|emailInDomainSuffixes|Coleção String|Um endereço de email sem um sufixo que corresponde a qualquer uma dessas strings será considerado fora do domínio.|
...
|faceTimeBlocked|Booliano|Indica se o usuário será ou não impedido de usar FaceTime.|
...
|iTunesBlockRadio|Booliano|Indica se o usuário será ou não impedido de usar iTunes Radio quando o dispositivo estiver no modo supervisionado (iOS 9.3 e posterior).|
...
|keyboardBlockDictation|Booliano|Indica se o usuário será ou não impedido de usar entrada de ditado quando o dispositivo estiver no modo supervisionado.|
...
|lockScreenBlockPassbook|Booliano|Indica se o usuário será ou não impedido de usar Passbook quando o dispositivo estiver bloqueado|
...
|mediaContentRatingApps|Cadeia de caracteres|Configurações de classificação de conteúdo de mídia Os valores possíveis são: `allAllowed`, `allBlocked`, `agesAbove4`, `agesAbove9`, `agesAbove12`, `agesAbove17`.|
...
|passcodeRequiredType|Cadeia de caracteres|Tipo de senha necessário. Os valores possíveis são: `deviceDefault`, `alphanumeric`, `numeric`.|
...
|safariBlockAutofill|Booliano|Indica se o usuário será ou não impedido de usar preenchimento automático no Safari.|
|safariBlockJavaScript|Booliano|Indica se o JavaScript deve ou não ser bloqueado no Safari.|
|safariBlockPopups|Booliano|Indica se as popups devem ou não ser bloqueadas no Safari.|
|safariBlocked|Booliano|Indica se o usuário será ou não impedido de usar Safari.|
|safariCookieSettings|Cadeia de caracteres|Configurações de cookie do Safari. Os valores possíveis são: `browserDefault`, `blockAlways`, `allowCurrentWebSite`, `allowFromWebsitesVisited`, `allowAlways`.|
|safariManagedDomains|Coleção String|As URLs correspondentes aos padrões indicados aqui serão consideradas gerenciadas.|
|safariPasswordAutoFillDomains|Coleção String|Os usuários podem salvar senhas no Safari somente de URLs correspondentes aos padrões indicados aqui. Aplica-se a dispositivos no modo supervisionado (iOS 9.3 ou posterior).|
...
|userStatuses|Coleção [deviceConfigurationUserStatus](../resources/intune_deviceconfig_deviceconfigurationuserstatus.md)|Status da instalação da configuração do dispositivo por usuário. Herdado de [deviceConfiguration](../resources/intune_deviceconfig_deviceconfiguration.md)|
