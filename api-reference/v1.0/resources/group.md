...
|preferredLanguage|Cadeia de caracteres|O idioma preferencial para um Microsoft 365 grupo. Deve seguir o código ISO 639-1; por exemplo, `en-US`. <br><br>Retornado por padrão. Suporta `$filter` (`eq`, `ne`, `NOT`, `ge`, `le`, `in`, `startsWith`). |
...
|owners|Coleção [directoryObject](directoryobject.md)|Os proprietários do grupo. Os proprietários são um conjunto de usuários não administradores e que têm permissão para modificar esse objeto. Limitado a 100 proprietários. Anulável. Se esta propriedade não for especificada ao criar um grupo do Microsoft 365, o usuário de chamada será automaticamente atribuído como o proprietário do grupo. Suporta o `$expand`.|
