...
description: 'Adiciona linhas ao final de uma tabela. '
...
As solicitações assíncrona serão úteis se a solicitação levar mais tempo do que o esperado. Observe que `Workbook-Session-Id` o header é necessário ao emissão de solicitações assíncronas. O usuário precisa criar [sessão antes](./workbook-createsession.md) de usar recursos de API avançados assíncrono. O header `Prefer:respond-async` também é necessário nas solicitações assíncrona.

Para recursos assíncronos, o usuário geralmente precisa emitir duas ou três solicitações: esta solicitação, [Obter workbookOperation](./workbookoperation-get.md)e, opcionalmente, [Obter tableRowOperationResult](./workbook-tablerowoperationresult.md).
