...
> **Nota:** Para listar os membros de um grupo de associação oculto, a _permissão Member.Read.Hidden_ é necessária.
...
Para filtrar os resultados no tipo OData, como `microsoft.graph.user` ou `microsoft.graph.group`, você deve usar os [parâmetros de consulta avançados](/graph/aad-advanced-queries). Ou seja, o **cabeçalho ConsistencyLevel** definido como e `eventual` a cadeia de caracteres `$count=true` de consulta.
...
### <a name="example-3-use-the-microsoftgraphgroup-odata-cast-to-get-only-members-that-are-groups"></a>Exemplo 3: Usar a conversão OData microsoft.graph.group para obter apenas membros que são grupos
...
### <a name="example-5-use-odata-cast-and-filter-to-get-user-membership-in-groups-with-a-display-name-that-starts-with-a-including-a-count-of-returned-objects"></a>Exemplo 5: Usar conversão e conversão de OData $filter para obter associação de usuário em grupos com um nome de exibição que começa com "A", incluindo uma contagem de objetos retornados
