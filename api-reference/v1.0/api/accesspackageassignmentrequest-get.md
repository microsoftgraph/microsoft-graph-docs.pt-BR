...
Este método dá suporte ao parâmetro de consulta OData para `$expand` expandir as relações, para recuperar o , `accessPackage` e `requestor` `assignment` .  Por exemplo, para recuperar o destino da atribuição do pacote de acesso, `$expand=assignment($expand=target)` inclua na consulta.  Para obter informações gerais, acesse [Parâmetros de consulta OData](/graph/query-parameters).
