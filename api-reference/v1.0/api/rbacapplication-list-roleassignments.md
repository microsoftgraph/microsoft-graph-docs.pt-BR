...
Essa operação requer `$filter` o parâmetro de consulta para consultar instâncias específicas de atribuições de função. Você pode filtrar as `roleDefinitionId` propriedades `principalId` ou. A `roleDefinitionId` propriedade pode ser uma ID de objeto de função ou um **templateId**. Para obter informações gerais, acesse [Parâmetros de consulta OData](/graph/query-parameters).
