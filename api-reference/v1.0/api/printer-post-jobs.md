...
No corpo da solicitação, fornece uma representação JSON de um [objeto printJob.](../resources/printjob.md) O objeto printJob deve conter apenas uma **propriedade de configuração.** Todas as propriedades de **configuração** são anuladas. Todos os outros campos, incluindo IDs de trabalho e documento, são definidos automaticamente durante a criação de recursos e não devem ser fornecidos na solicitação.
