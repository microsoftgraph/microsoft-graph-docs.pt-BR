...
A solicitação a seguir atualiza o logotipo da faixa para `fr` a localização. Usando PUT, se a localização fr não existir, um `404 not found` erro será retornado. Se a carga contiver uma propriedade ou um `id` header **content-Language** e eles não corresponderem `id` na URL, um `Bad Request` erro será retornado.
