...
|useDeviceContext|Booliano|Indica se uma MSI de modo duplo deve ser instalada no contexto do dispositivo. Se for true, o aplicativo será instalado para todos os usuários. Se for false, o aplicativo será instalado por usuário. Se for nulo, o serviço usará o contexto de instalação padrão do pacote MSI. No caso de MSI de modo duplo, esse padrão será por usuário.  Não pode ser definido para aplicativos de modo não duplo.  Não é possível alterar após a criação inicial do aplicativo.|
