...
|dateAndTimeBlockChanges|Boolean|Indica se deve ou não bloquear a alteração de data e hora no modo KNOX.|
...
|requiredPasswordComplexity|[androidRequiredPasswordComplexity](../resources/intune-deviceconfig-androidrequiredpasswordcomplexity.md)|Indica a complexidade de senha necessária no Android. Um de: NONE, LOW, MEDIUM, HIGH. Essa é uma API direcionada ao Android 11+. Os valores possíveis são: `none`, `low`, `medium`, `high`.|
