...
|[ação extendFeatureUpdatesPause](../api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendfeatureupdatespause.md)|Nenhum|Estenda o recurso Atualizações pausar para um anel Windows Update para Empresas.|
|[ação extendQualityUpdatesPause](../api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendqualityupdatespause.md)|Nenhum|Estender a qualidade Atualizações pausar para um anel Windows Update para Empresas.|
...
|skipChecksBeforeRestart|Booleano|Defina para ignorar todas as verificações antes da reinicialização: Nível da bateria = 40%, Presença do usuário, Exibição Necessária, Modo de Apresentação, Modo de tela inteira, estado da chamada telefônica, modo de jogo etc. |
...
|qualityUpdatesPauseStartDate|Data|Data de início Atualizações pausa de qualidade. Essa propriedade é somente leitura.|
|featureUpdatesPauseStartDate|Data|O recurso Atualizações pausar a data de início. Essa propriedade é somente leitura.|
...
|qualityUpdatesWillBeRolledBack|Booleano|Especifica se a qualidade deve ser revertida Atualizações no próximo check-in do dispositivo|
|featureUpdatesWillBeRolledBack|Booleano|Especifica se a reversão do recurso Atualizações no próximo check-in do dispositivo|
|qualityUpdatesRollbackStartDateTime|DateTimeOffset|Data e hora Atualizações início da reversão de qualidade|
|featureUpdatesRollbackStartDateTime|DateTimeOffset|Data e hora Atualizações início da reversão do recurso|
...
|postponeRebootUntilAfterDeadline|Booleano|Especifica se o dispositivo deve aguardar até o prazo para reinicializar fora do horário ativo|
...
|userPauseAccess|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Especifica se o acesso do usuário final deve ser habilitado para pausar atualizações de software. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
|userWindowsUpdateScanAccess|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Especifica se o acesso do usuário deve ser desabilitado para verificar Windows Update. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
...
|allowWindows11Upgrade|Booleano|Permitir que dispositivos Windows 10 qualificados atualizem para a versão mais recente do Windows 11.|
...
|deviceUpdateStates|[coleção windowsUpdateState](../resources/intune-deviceconfig-windowsupdatestate.md)|Estados do dispositivo de configuração do Windows Update para Empresas. Esta coleção pode conter um máximo de 500 elementos.|
