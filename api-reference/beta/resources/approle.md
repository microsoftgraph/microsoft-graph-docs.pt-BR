...
|:---------------|:--------|:----------|
|allowedMemberTypes|Coleção String|Especifica se essa função de aplicativo pode ser atribuída a usuários e grupos ( `["User"]`definindo como ), para outro aplicativo ( `["Application"]`definindo como , ou ambos (definindo como `["User", "Application"]`). Funções de aplicativo que suportam a atribuição a entidades de serviço de outros aplicativos também são [conhecidas como permissões de aplicativo](/graph/auth/auth-concepts#microsoft-graph-permissions). O valor "Application" só é suportado para funções de aplicativo definidas em **entidades de** aplicativo. |
