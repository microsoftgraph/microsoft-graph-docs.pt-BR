...
|taskManagerBlockEndTask|Booleano|Especifique se não administradores podem usar o Gerenciador de Tarefas para encerrar tarefas.|
|energySaverOnBatteryThresholdPercentage|Int32|Essa configuração permite que você especifique o nível de carga da bateria no qual a Economia de Energia está ativada. Durante a bateria, a Economia de Energia é ativada automaticamente em (e abaixo) do nível de carga da bateria especificado. Intervalo de entrada válido (0-100). Valores válidos de 0 a 100|
|energySaverPluggedInThresholdPercentage|Int32|Essa configuração permite que você especifique o nível de carga da bateria no qual a Economia de Energia está ativada. Enquanto conectado, a Economia de Energia é ativada automaticamente em (e abaixo) do nível de carga da bateria especificado. Intervalo de entrada válido (0-100). Valores válidos de 0 a 100|
|powerLidCloseActionOnBattery|[powerActionType](../resources/intune-deviceconfig-poweractiontype.md)|Essa configuração especifica a ação que o Windows executa quando um usuário fecha a tampa em um computador móvel durante a bateria. Os valores possíveis são: `notConfigured`, `noAction`, `sleep`, `hibernate`, `shutdown`.|
|powerLidCloseActionPluggedIn|[powerActionType](../resources/intune-deviceconfig-poweractiontype.md)|Essa configuração especifica a ação que o Windows executa quando um usuário fecha a tampa em um computador móvel enquanto está conectado. Os valores possíveis são: `notConfigured`, `noAction`, `sleep`, `hibernate`, `shutdown`.|
|powerButtonActionOnBattery|[powerActionType](../resources/intune-deviceconfig-poweractiontype.md)|Essa configuração especifica a ação que o Windows executa quando um usuário pressiona o botão Ligar/Desligar durante a bateria. Os valores possíveis são: `notConfigured`, `noAction`, `sleep`, `hibernate`, `shutdown`.|
|powerButtonActionPluggedIn|[powerActionType](../resources/intune-deviceconfig-poweractiontype.md)|Essa configuração especifica a ação que o Windows executa quando um usuário pressiona o botão Ligar/Desligar enquanto está conectado. Os valores possíveis são: `notConfigured`, `noAction`, `sleep`, `hibernate`, `shutdown`.|
|powerSleepButtonActionOnBattery|[powerActionType](../resources/intune-deviceconfig-poweractiontype.md)|Essa configuração especifica a ação que o Windows executa quando um usuário pressiona o botão Suspender durante a bateria. Os valores possíveis são: `notConfigured`, `noAction`, `sleep`, `hibernate`, `shutdown`.|
|powerSleepButtonActionPluggedIn|[powerActionType](../resources/intune-deviceconfig-poweractiontype.md)|Essa configuração especifica a ação que o Windows executa quando um usuário pressiona o botão Suspender enquanto está conectado. Os valores possíveis são: `notConfigured`, `noAction`, `sleep`, `hibernate`, `shutdown`.|
|powerHybridSleepOnBattery|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Essa configuração permite que você desative o  sono híbrido durante a bateria. Se você definir essa configuração como desabilitada, um hiberfile não será gerado quando o sistema fizer a transição para  sleep (Stand By). Se você definir essa configuração para habilitar ou não definir essa configuração de política, os usuários controlarão essa configuração. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
|powerHybridSleepPluggedIn|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Essa configuração permite que você desative o  sono híbrido enquanto estiver conectado. Se você definir essa configuração como desabilitada, um hiberfile não será gerado quando o sistema fizer a transição para  sleep (Stand By). Se você definir essa configuração para habilitar ou não definir essa configuração de política, os usuários controlarão essa configuração. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
...
|enableAutomaticRedeployment|Booliano|Permitir que os usuários com direitos administrativos excluam todos os dados e configurações do usuário usando CTRL + Win + R na tela de bloqueio do dispositivo para que o dispositivo possa ser automaticamente configurado e registrado novamente no gerenciamento.|
|microsoftAccountSignInAssistantSettings|[signInAssistantOptions](../resources/intune-deviceconfig-signinassistantoptions.md)|Controla o serviço NT do NT Sign-In Conta da Microsoft (wlidsvc). Os valores possíveis são: `notConfigured` e `disabled`.|
|authenticationAllowSecondaryDevice|Booleano|Permite que dispositivos de autenticação secundários funcionem com o Windows.|
|authenticationWebSignIn|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Indica se o Provedor de Credenciais da Web será habilitado ou não. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
|authenticationPreferredAzureADTenantDomainName|Cadeia de Caracteres|Especifica o domínio preferencial entre os domínios disponíveis no Azure AD locatário.|
|cryptographyAllowFipsAlgorithmPolicy|Booleano|Especifique se deseja permitir ou não a política FIPS (Federal Information Processing Standard).|
|displayAppListWithGdiDPIScalingTurnedOn|Conjunto de cadeias de caracteres|Lista de aplicativos herdados que têm o Dimensionamento de DPI GDI ativado.|
|displayAppListWithGdiDPIScalingTurnedOff|Conjunto de cadeias de caracteres|Lista de aplicativos herdados que têm o dimensionamento de DPI GDI desativado.|
...
|experienceDoNotSyncBrowserSettings|[browserSyncSetting](../resources/intune-deviceconfig-browsersyncsetting.md)|Permitir ou impedir a sincronização das configurações do Microsoft Edge Browser. Opção para administradores de TI impedirem a sincronização entre dispositivos, mas permitir a substituição do usuário. Os valores possíveis são: `notConfigured`, `blockedWithUserOverride`, `blocked`.|
|messagingBlockSync|Booleano|Indica se a mensagem de texto deve ou não ser bloqueiada e restaurada e mensagens em todos os lugares.|
|messagingBlockMMS|Booleano|Indica se a funcionalidade de envio/recebimento do MMS deve ou não ser bloqueado no dispositivo.|
|messagingBlockRichCommunicationServices|Booleano|Indica se a funcionalidade de envio/recebimento do RCS deve ou não ser bloqueado no dispositivo.|
|printerNames|Coleção de cadeias de caracteres|Provisione impressoras automaticamente com base em seus nomes (nomes de host de rede).|
|printerDefaultName|String|Nome (nome do host de rede) de uma impressora instalada.|
|printerBlockAddition|Booleano|Impedir a instalação do usuário de impressoras adicionais nas configurações de impressoras.|
...
|searchDisableUseLocation|Booleano|Especifica se a pesquisa pode usar informações de localização.|
|searchDisableLocation|Booliano|Especifica se a pesquisa pode usar informações de localização.|
...
|searchBlockWebResults|Booleano|Indica se a pesquisa na Web deve ou não ser bloqueado.|
|findMyFiles|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Controla se o usuário pode configurar a pesquisa para o modo Localizar Meus Arquivos, que pesquisa arquivos em discos rígidos secundários e também fora do perfil do usuário. Localizar Meus Arquivos não permite que os usuários pesquisem arquivos ou locais aos quais eles não têm acesso. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
|securityBlockAzureADJoinedDevicesAutoEncryption|Booleano|Especifique se deseja permitir a criptografia automática de dispositivo durante o OOBE quando o dispositivo Azure AD ingressado (somente desktop).|
...
|systemTelemetryProxyServer|Cadeia de Caracteres|Obtém ou define o FQDN (nome de domínio totalmente qualificado) ou o endereço IP de um servidor proxy para encaminhar as solicitações de Telemetria e Experiências do Usuário Conectado.|
|edgeTelemetryForMicrosoft365Analytics|[edgeTelemetryMode](../resources/intune-deviceconfig-edgetelemetrymode.md)|Especifica que tipo de dados de telemetria (nenhum, intranet, Internet, ambos) são enviados para o Microsoft 365 Analytics. Os valores possíveis são: `notConfigured`, `intranet`, `internet`, `intranetAndInternet`.|
|inkWorkspaceAccess|[inkAccessSetting](../resources/intune-deviceconfig-inkaccesssetting.md)|Controla o acesso do usuário ao workspace de tinta, da área de trabalho e acima da tela de bloqueio. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
|inkWorkspaceAccessState|[stateManagementSetting](../resources/intune-deviceconfig-statemanagementsetting.md)|Controla o acesso do usuário ao workspace de tinta, da área de trabalho e acima da tela de bloqueio. Os valores possíveis são: `notConfigured`, `blocked`, `allowed`.|
|inkWorkspaceBlockSuggestedApps|Booleano|Especifique se deseja mostrar as sugestões de aplicativo recomendadas no workspace de tinta.|
...
|bluetoothBlockPromptedProximalConnections|Booliano|Se os usuários devem ou não usar Emparelhamento Rápido e outros cenários baseados em proximidade.|
...
|edgeFavoritesListLocation|Cadeia de Caracteres|O local da lista de favoritos a ser provisionada. Pode ser um arquivo local, rede local ou local http.|
|edgeBlockEditFavorites|Booliano|Indica se o usuário será ou não bloqueado de fazer alterações nos Favoritos.|
|edgeNewTabPageURL|Cadeia de Caracteres|Especifique a página aberta quando novas guias forem criadas.|
|edgeHomeButtonConfiguration|[edgeHomeButtonConfiguration](../resources/intune-deviceconfig-edgehomebuttonconfiguration.md)|Faz com que o botão Página Inicial oculte, carregue a página Inicial padrão, carregue uma página nova guia ou uma URL personalizada|
|edgeHomeButtonConfigurationEnabled|Booleano|Habilite a configuração do botão Página Inicial.|
...
|edgeBlockSideloadingExtensions|Booleano|Indica se o usuário pode fazer sideload de extensões.|
|edgeRequiredExtensionPackageFamilyNames|Coleção de cadeias de caracteres|Especifique a lista de nomes de família de pacotes de extensões do navegador que são necessárias e não podem ser desativadas pelo usuário.|
|edgeBlockPrinting|Booleano|Configure o Edge para permitir ou bloquear a impressão.|
|edgeFavoritesBarVisibility|[visibilitySetting](../resources/intune-deviceconfig-visibilitysetting.md)|Obtenha ou defina um valor que especifica se a barra de favoritos deve ser sempre visível ou oculta em qualquer página. Os valores possíveis são: `notConfigured`, `hide`, `show`.|
|edgeBlockSavingHistory|Booleano|Configure o Edge para permitir que o histórico de navegação seja salvo ou nunca salve o histórico de navegação.|
|edgeBlockFullScreenMode|Booleano|Permitir ou impedir que o Edge entre no modo de tela inteira.|
|edgeBlockWebContentOnNewTabPage|Booleano|Configure para carregar uma página em branco no Edge em vez da página padrão nova guia e impedir que os usuários a alterem.|
|edgeBlockTabPreloading|Booleano|Configure se o Edge pré-carregará a página da nova guia na inicialização do Windows.|
|edgeBlockPrelaunch|Booleano|Decida se o Microsoft Edge está pré-inicializado na inicialização do Windows.|
...
|edgePreventCertificateErrorOverride|Booleano|Permitir ou impedir que os usuários substituam erros de certificado.|
|edgeKioskModeRestriction|[edgeKioskModeRestrictionType](../resources/intune-deviceconfig-edgekioskmoderestrictiontype.md)|Controla como as configurações do Microsoft Edge são restritas com base no modo de configuração de quiosque. Os valores possíveis são: `notConfigured`, `digitalSignage`, `normalMode`, `publicBrowsingSingleApp`, `publicBrowsingMultiApp`.|
|edgeKioskResetAfterIdleTimeInMinutes|Int32|Especifica o tempo em minutos da última atividade do usuário antes da redefinição do quiosque do Microsoft Edge.  Os valores válidos são 0-1440. O padrão é 5. 0 indica que não há redefinição. Valores válidos de 0 a 1440|
...
|cellularData|[configurationUsage](../resources/intune-deviceconfig-configurationusage.md)|Se o canal de dados da rede celular deve ou não ser permitido no dispositivo. Se não estiver configurado, o canal de dados da rede celular será permitido e o usuário poderá desativá-lo. Os valores possíveis são: `blocked`, `required`, `allowed`, `notConfigured`.|
...
|defenderScheduleScanEnableLowCpuPriority|Booleano|Quando habilitada, a baixa prioridade da CPU será usada durante as verificações agendadas.|
|defenderDisableCatchupQuickScan|Booleano|Quando bloqueado, as verificações de atualização para verificações rápidas agendadas serão desativadas.|
|defenderDisableCatchupFullScan|Booleano|Quando bloqueado, as verificações de atualização das verificações completas agendadas serão desativadas.|
...
|defenderCloudExtendedTimeout|Int32|Extensão de tempo limite para verificação de arquivos pela nuvem. Valores válidos de 0 a 50|
|defenderCloudExtendedTimeoutInSeconds|Int32|Extensão de tempo limite para verificação de arquivos pela nuvem. Valores válidos de 0 a 50|
...
|defenderPotentiallyUnwantedAppAction|[defenderPotentiallyUnwantedAppAction](../resources/intune-deviceconfig-defenderpotentiallyunwantedappaction.md)|Obtém ou define a ação do Defender a ser tomada no PUA (Aplicativo Potencialmente Indesejado), que inclui software com comportamentos de injeção de anúncios, agrupamento de software, solicitação persistente para pagamento ou assinatura, etc. O Defender alerta o usuário quando o PUA está sendo baixado ou tenta se instalar. Adicionado no Windows 10 para área de trabalho. Os valores possíveis são: `deviceDefault`, `block`, `audit`.|
|defenderPotentiallyUnwantedAppActionSetting|[defenderProtectionType](../resources/intune-deviceconfig-defenderprotectiontype.md)|Obtém ou define a ação do Defender a ser tomada no PUA (Aplicativo Potencialmente Indesejado), que inclui software com comportamentos de injeção de anúncios, agrupamento de software, solicitação persistente para pagamento ou assinatura, etc. O Defender alerta o usuário quando o PUA está sendo baixado ou tenta se instalar. Adicionado no Windows 10 para área de trabalho. Os valores possíveis são: `userDefined`, `enable`, `auditMode`, `warn`, `notConfigured`.|
...
|defenderBlockOnAccessProtection|Booleano|Permite ou não Windows Defender funcionalidade de Proteção de Acesso.|
...
|lockScreenActivateAppsWithVoice|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Essa configuração de política especifica se os aplicativos do Windows podem ser ativados por voz enquanto o sistema está bloqueado. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
...
|passwordMinimumAgeInDays|Int32|Essa configuração de segurança determina o período de tempo (em dias) que uma senha deve ser usada antes que o usuário possa alterá-la. Valores válidos de 0 a 998|
...
|privacyDisableLaunchExperience|Booleano|Essa política impede que a experiência de privacidade seja iniciada durante o logon do usuário para usuários novos e atualizados.|
...
|privacyBlockPublishUserActivities|Booleano|Bloqueia as experiências compartilhadas/descoberta de recursos usados recentemente no alternador de tarefas etc.|
|privacyBlockActivityFeed|Booleano|Bloqueia o uso de serviços de fala baseados em nuvem para aplicativos Cortana, Ditado ou Store.|
|activateAppsWithVoice|[Capacitação](../resources/intune-deviceconfig-enablement.md)|Especifica se os aplicativos do Windows podem ser ativados por voz. Os valores possíveis são: `notConfigured`, `enabled`, `disabled`.|
...
|windowsSpotlightConfigureOnLockScreen|[windowsSpotlightEnablementSettings](../resources/intune-deviceconfig-windowsspotlightenablementsettings.md)|Especifica o tipo de Spotlight. Os valores possíveis são: `notConfigured`, `disabled`, `enabled`.|
...
|edgeBlockSearchEngineCustomization|Booleano|Indica se o usuário deve ou não bloquear a adição de um novo mecanismo de pesquisa ou a alteração do mecanismo de pesquisa padrão.|
|edgeBlockSendingIntranetTrafficToInternetExplorer|Boolean|Indica se o tráfego da intranet deve ou não ser alternado do Edge para o Internet Explorer. Observação: o nome desta propriedade é enganoso; a propriedade está obsoleta, use EdgeSendIntranetTrafficToInternetExplorer.|
|edgeSendIntranetTrafficToInternetExplorer|Booleano|Indica se o tráfego da intranet deve ou não ser alternado do Edge para o Internet Explorer.|
...
|tenantLockdownRequireNetworkDuringOutOfBoxExperience|Booleano|Se o dispositivo é necessário para se conectar à rede.|
...
|appManagementMSIAlwaysInstallWithElevatedPrivileges|Booleano|Essa configuração de política direciona o Windows Installer a usar permissões elevadas ao instalar qualquer programa no sistema.|
|dataProtectionBlockDirectMemoryAccess|Booleano|Essa configuração de política permite bloquear o DMA (acesso direto à memória) para todas as portas downstream PCI conectáveis a quente até que um usuário se conecte ao Windows.|
|appManagementPackageFamilyNamesToLaunchAfterLogOn|Conjunto de cadeias de caracteres|Lista de nomes de família de pacotes delimitados por ponto e vírgula de aplicativos do Windows. Os aplicativos do Windows listados devem ser iniciados após o logon.|
|uninstallBuiltInApps|Booliano|Indica se deseja ou não desinstalar uma lista fixa de aplicativos internos do Windows.|
|configureTimeZone|Cadeia de Caracteres|Especifica o fuso horário a ser aplicado ao dispositivo. Esse é o nome padrão do Windows para o fuso horário de destino.|
