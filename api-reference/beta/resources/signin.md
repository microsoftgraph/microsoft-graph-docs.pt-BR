...
|:---------------|:--------|:----------|
|alternateSignInName|String|A identidade de login alternativa sempre que você usa o número de telefone para entrar. Oferece `$filter` suporte ( e somente `eq` `startsWith` operadores).|
...
|clientAppUsed|Cadeia de caracteres| Identifica o cliente usado para a atividade de entrada. Os clientes de autenticação moderna `Browser` incluem e `modern clients` . Os clientes de autenticação `Exchange Active Sync` herdou `IMAP` incluem , `MAPI` , , , e `SMTP` `POP` `other clients` . Suporta `$filter` ( `eq` somente operador). |
...
|id|String|O identificador que representa a atividade de login. Suporta `$filter` ( `eq` somente operador).|
