...
|status|Cadeia de caracteres|O estado do ciclo de vida da extensão do esquema. Os estados possíveis **são InDevelopment,** **Available** e **Deprecated**. Definir automaticamente **como InDevelopment na** criação. [As extensões de esquema](/graph/extensibility-overview#schema-extensions) fornece mais informações sobre as possíveis transições de estado e comportamentos. Suporta `$filter` (`eq`).|
|targetTypes|Coleção de cadeias de caracteres|Conjunto de tipos Graph microsoft (que podem dar suporte a extensões) aos quais a extensão de esquema pode ser aplicada. Selecione **administrativeUnit**, **contact**, **device**, **event**, **group**, **message**, **organization**, **post**, or **user**.|
