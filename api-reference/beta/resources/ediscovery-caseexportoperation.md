...
|:---|:---|:---|
|[getDownloadUrl](../api/ediscovery-caseexportoperation-getdownloadurl.md)|String| Retorna a URL da exportação.|
...
|azureBlobContainer|String| O nome do local de armazenamento do Azure onde a exportação será armazenada. Isso só se aplica às exportações armazenadas em seu próprio local de armazenamento do Azure. |
|azureBlobToken|String| O token SAS para o local de armazenamento do Azure.  Isso só se aplica às exportações armazenadas em seu próprio local de armazenamento do Azure. |
...
|exportOptions|microsoft.graph.ediscovery.exportOptions| As opções fornecidas para a exportação. Consulte [reviewSet: export](../api/ediscovery-reviewset-export.md) for more details. Os valores possíveis são: `originalFiles`, `text`, `pdfReplacement`, `fileInfo`, `tags`.|
|exportStructure|microsoft.graph.ediscovery.exportFileStructure|As opções fornecidas que especificam a estrutura da exportação. Consulte [reviewSet: export](../api/ediscovery-reviewset-export.md) for more details. Os valores possíveis são: `none`, `directory`, `pst`.|
...
|Member| Descrição |
|:---|:---|
|originalFiles| Incluir cópias dos arquivos originais - exclua essa opção ao gerar relatórios somente |
...
|fileInfo| Inclua o arquivo de verão e de carga - isso sempre deve ser incluído. |
|tags| Inclua marcas de documento que foram aplicadas durante a revisão no arquivo de carga. |

### <a name="exportfilestructure-values"></a>valores exportFileStructure

|Member| Descrição |
|:---|:---|
|directory| Mapeia para a estrutura de diretório condensada comumente usada por ferramentas de Descoberta e. Todos os arquivos são exportados para um arquivo raiz chamado NativeFiles. |
