...
|assignmentState|Cadeia de caracteres  |Obrigatório. O estado da atribuição. Os valores possíveis são: `Eligible` (para atribuição qualificada),  `Active` (se for atribuída diretamente), `Active` (por administradores ou ativada em uma atribuição qualificada pelos usuários).|
