...
\* Esse cenário só tem suporte para a seguinte chamada: `GET /users/{user-id | user-principal-name}/chats`
...
Esse método dá suporte aos `$expand` parâmetros de consulta (**members** e **lastMessagePreview** ) `$filter` e [OData](/graph/query-parameters) para ajudar a personalizar a resposta.
...
O exemplo a seguir mostra uma solicitação para listar chats junto com a visualização da última mensagem enviada no chat. Comparar na `createdDateTime` visualização permite `lastMessageReadDateTime` `viewpoint` que o chamador determine se o usuário leu todas as mensagens em um chat.
...
### <a name="example-4-list-all-chats-that-have-a-member-with-a-specific-display-name"></a>Exemplo 4: listar todos os chats que têm um membro com um nome de exibição específico
