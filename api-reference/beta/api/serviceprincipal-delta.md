...
Get recentemente criado, atualizado ou excluído entidades de serviço sem precisar realizar uma leitura completa do conjunto de recurso inteiro. Consulte [Usando Delta consulta](/graph/delta-query-overview) para obter detalhes.
...
Para começar a controlar alterações, você deve fazer uma solicitação, incluindo a função delta no recurso servicePrincipal. 
...
Se tiver êxito, este método retornará `200 OK` objeto de coleção [servicePrincipal](../resources/serviceprincipal.md) e código de resposta no corpo da resposta. A resposta também inclui uma URL de nextLink ou uma URL de deltaLink. 
