...
Este método oferece suporte a alguns dos parâmetros de consulta OData para ajudar a personalizar a resposta. Por exemplo, para recuperar uma política de atribuição de pacote do Access com um nome de `$filter=displayName eq 'Employee sales support'` exibição especificado, inclua na consulta. Para obter informações gerais, confira [parâmetros de consulta OData](/graph/query-parameters).
