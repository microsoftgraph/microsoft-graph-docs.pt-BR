...
Este método dá suporte `$select` aos `$filter` parâmetros de consulta , e OData para ajudar a personalizar `$expand` a resposta. Por exemplo, para recuperar quem solicitou a adição de um recurso a um catálogo, `$expand=requestor` inclua na consulta. Para obter informações gerais, acesse [Parâmetros de consulta OData](/graph/query-parameters).
...
Se tiver êxito, este método retornará um código de resposta e uma coleção de `200 OK` [objetos accessPackageResourceRequest](../resources/accesspackageresourcerequest.md) no corpo da resposta.
