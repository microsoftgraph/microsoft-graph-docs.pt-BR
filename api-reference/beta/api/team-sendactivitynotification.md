...
Este exemplo mostra como você pode enviar uma notificação de feed de atividade para todos os membros da equipe. Este exemplo é semelhante aos exemplos anteriores. No entanto, nesse caso, o `recipient` é [um teamMembersNotificationRecipient](../resources/teammembersnotificationrecipient.md). Observe que o `teamId` especificado no deve `recipient` corresponder ao `teamId` especificado na URL de solicitação.
