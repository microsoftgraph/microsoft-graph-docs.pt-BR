...
Atualize as propriedades de um `userExperienceAnalyticsDeviceScope` objeto.
...
No corpo da solicitação, forneça uma representação JSON do `userExperienceAnalyticsDeviceScope` objeto.

A tabela a seguir mostra as propriedades que são necessárias ao criar o `userExperienceAnalyticsDeviceScope`.
...
Se tiver êxito, este método retornará um `200 OK` código de resposta e um objeto `userExperienceAnalyticsDeviceScope` atualizado no corpo da resposta.
