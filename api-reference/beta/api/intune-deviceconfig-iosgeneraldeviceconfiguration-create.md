...
|proximityBlockSetupToNewDevice|Boolean|Indica se o prompt será habilitado ou não para configurar dispositivos próximos com um dispositivo supervisionado.|
...
|softwareUpdatesEnforcedDelayInDays|Int32|Define por quantos dias uma atualização de software será des controlada para um dispositivo supervisionado. Valores válidos de 0 a 90|
...
|pkiBlockOTAUpdates|Boolean|Indica se as atualizações PKI estão bloqueadas ou não. Definir essa restrição como false não desabilita verificações CRL e OCSP (iOS 7.0 e posterior).|
...
|applePersonalizedAdsBlocked|Boolean|Limita o anúncio personalizado da Apple quando verdadeiro. Disponível no iOS 14 e posterior.|
