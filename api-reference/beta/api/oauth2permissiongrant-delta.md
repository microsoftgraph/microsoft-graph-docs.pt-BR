...
- Se uma URL `@odata.nextLink` é retornada, existem páginas de dado adicionais a serem recuperadas na sessão. O **oauth2permissiongrant** continua fazendo `@odata.nextLink` solicitações usando a URL até que uma `@odata.deltaLink` URL seja incluída na resposta.
